...
| 계약 보관 문서 (완료본·감사추적인증서 R2 사본, 플랫폼 밖 계약서 업로드) | 높음 | 행 소유 워크스페이스 단일 판정 + presigned 302 (§3.3) |
...
**계약 보관 문서 (v0.5.0.0)** — 같은 버킷의 별도 키 공간(`contract-archives/…`)에 두 부류가 산다: 전자서명 완료본·감사추적인증서의 **우리 사본**(cron 하이드레이션)과 플랫폼 밖 계약서의 **수동 업로드**. 세 라우트(`/api/contract-archives/presign` · `/{id}/complete` · `/{id}/download`)는 첨부 라우트의 미러이며 3중 세션 게이트(`auth`/`isSessionRevoked`/`isEmailUnverified`)에 더해 `isPgMembershipBlocked` 를 **인라인으로 배선**한다(신규 `/api` 라우트는 프록시 매처 밖이라 인라인이 유일한 게이트다). **ACL 의 SSOT 는 행 소유 워크스페이스 하나**다 — 보관함 행은 생성 시점에 이미 당사자별(buyer 1행 + PG 1행)로 갈라지므로 다운로드·삭제가 서명 계약의 당사자 판정을 **다시 하지 않는다**(판정을 둘로 두면 갈릴 수 있다). 남의 행에는 403 이 아니라 **404**, 비-uuid 도 404 로 맞춘다 — 상태 코드가 존재 오라클이 되지 않게(`getForActor` 의 ACL-먼저 규율과 같은 이유이고, 비-uuid 를 그대로 넘기면 Postgres 22P02 가 처리되지 않은 500 으로 샌다). presign 정책: PDF 전용 · 건당 30MB · 워크스페이스당 업로드 200건 캡 · PUT TTL 10분이며, phase-2 가 `head` 크기 + 첫 4KB 매직바이트로 **클라이언트 자기신고를 재검증**하고 불일치면 객체·행을 함께 폐기한다. 삭제는 `source='upload'` 만 — 자동 보관본은 서버(`deleteUpload`)가 거부하고 UI 의 `canDelete` 는 그 파생일 뿐이다(보존 원칙). **새 아웃바운드 축**: 하이드레이션 fetch 는 앞선 공급자 문서 경로들과 달리 사용자 브라우저를 302 로 보내지 않고 **우리 VM 에서 직접 나간다** — `https://` 강제 + 청크 누적 실바이트 30MB 캡(`content-length` 사전 게이트만으로는 chunked 응답이 헤더 없이 그냥 통과한다) + 15초 데드라인이 방어이고, **호스트 핀은 없다**(완료본 URL 이 API 호스트가 아니라 열거 불가능한 S3 presigned 라서 — 템플릿 PDF 를 302 대신 프록시하는 것과 같은 이유). 가드: `app/api/contract-archives/__tests__/*`, `lib/server/services/__tests__/contract-archive.test.ts`.
