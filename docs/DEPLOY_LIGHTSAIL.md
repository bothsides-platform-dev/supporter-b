...
- **Web Vitals** — 브라우저가 `POST /api/axiom` 으로 보내면 서버가 Axiom 에 넣는다(`app/api/axiom/route.ts`). 토큰·데이터셋은 **서버에서만** 읽고 `NEXT_PUBLIC_AXIOM_TOKEN`+`NEXT_PUBLIC_AXIOM_DATASET` **쌍이 있을 때만** 켜진다(next-axiom 시절 브라우저가 쓰던 그 쌍이라 같은 데이터셋에 쌓인다). 위 운영 로그용 `AXIOM_*` 로는 **폴백하지 않는다** — 비인증 엔드포인트라 폴백하면 익명 요청이 운영 로그 데이터셋에 쓸 수 있게 된다. 쌍이 없으면 수집을 조용히 버린다(204). 런타임 값이라 재빌드 없이 PM2 재시작으로 바뀐다. 이벤트 모양은 `{webVital:{…, entries:정규화}, _time, source:'web-vital', path}` — next-axiom 의 `route`·`platform` 필드는 없다.
...
세 종류의 알림이 운영자 슬랙 채널로 나간다(best-effort, 커밋 후 fire-and-forget — 실패해도 기능 무영향).
...
3. **견적·상담 주요 사건** — 기존 견적 요청의 초대 발송, 맞춤 상담의 최초 요청·PG사 검토 시작·거절·다음 PG사 요청, 견적 제출, 최종 선정. 실제 상태 변경이 커밋된 경우에만 보내며, 재호출로 기존 상담을 돌려주거나 이미 검토 중이거나 초대 발송 대상이 0건이면 보내지 않는다.
...
**계약 메시지**에는 견적번호·제목·이벤트·회차만 담긴다. **견적·상담 메시지**에는 견적번호·제목·이벤트·관련 PG사 이름을 담고, 견적 재제출은 회차를 덧붙인다. 초대 대상 PG사가 많으면 슬랙 본문 길이 안에서 앞쪽 이름을 표시하고 생략한 회사 수를 `외 N곳`으로 명시한다. 두 메시지 모두 금액·수수료·사업자번호를 담지 않는다. 가입 알림에는 워크스페이스명·가입자명·심사 링크가 담긴다.

> **웹훅 URL 은 채널 하나에 묶인다.** 계약·가입·견적 알림이 같은 채널에 모인다 — 나중에 나누려면 두 번째 환경변수를 추가하는 것이지 이 값을 재설정하는 게 아니다.
