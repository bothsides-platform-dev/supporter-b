...
- **Web Vitals** — 브라우저가 `POST /api/axiom` 으로 보내면 서버가 Axiom 에 넣는다(`app/api/axiom/route.ts`). 토큰·데이터셋은 **서버에서만** 읽고 필드별로 `NEXT_PUBLIC_AXIOM_TOKEN`/`NEXT_PUBLIC_AXIOM_DATASET` 가 우선, 없으면 위 `AXIOM_*` 로 폴백한다(next-axiom 시절과 같은 데이터셋 유지). 둘 다 없으면 수집을 조용히 버린다(204). 런타임 값이라 재빌드 없이 PM2 재시작으로 바뀐다. 이벤트 모양은 `{webVital:{…, entries:정규화}, _time, source:'web-vital', path}` — next-axiom 의 `route`·`platform` 필드는 없다.
