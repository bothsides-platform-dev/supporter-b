# 배포 런북 — AWS Lightsail + Caddy (Amazon Linux 2)
...
| 항목 | 값 | 근거 |
|---|---|---|
| OS | **Amazon Linux 2** | 사용자 선택. glibc 2.26 → Node 22 는 glibc-217 빌드로 우회(아래 §glibc) |
...
1. **Lightsail 인스턴스 생성**: 블루프린트 **Amazon Linux 2**, 플랜 **2GB**. SSH 키 등록.
...
```bash
# 1) SSH 접속 (Lightsail AL2 기본 사용자: ec2-user)
...
# 3) 1회 프로비저닝 — swap, Node22(glibc-217), pnpm, Docker, PM2, Caddy(systemd)
...
- Amazon Linux 2 는 ufw 가 없고(Debian 계열 도구), firewalld 도 Lightsail 이미지에서 기본 비활성. iptables 기본 정책은 ACCEPT 라 OCI 식 삽입도 불필요.
...
## glibc 주의 (Amazon Linux 2)

AL2 는 glibc 2.26 이고, 공식 Node 18+ 바이너리는 glibc 2.28+ 를 요구한다 (`version 'GLIBC_2.28' not found`). 그래서 bootstrap 은 nodejs.org 의 **비공식 `glibc-217` 빌드**(v18~v23 지원)를 받아 `/usr/local` 에 설치한다 — 2.26 에서 정상 실행된다.

- 버전 변경: `NODE_VERSION=22.x.x bash scripts/deploy/lightsail-bootstrap.sh`
- 비공식 빌드 의존이 싫으면 인스턴스를 **Amazon Linux 2023**(glibc 2.34)으로 재생성하고 bootstrap §3 을 표준 NodeSource(`dnf`) 설치로 교체하면 된다. 나머지 절차는 동일.

## 트러블슈팅

- **사이트 안 뜸 / TLS 안 됨**: 콘솔 방화벽 443 열렸는지(가장 흔함), `dig` 가 고정 IP 가리키는지, `journalctl -u caddy` 의 ACME 에러 확인.
- **`GLIBC_2.28 not found`**: bootstrap 이 글로벌 Node 를 덮어쓰지 못한 경우. `which node`(=`/usr/local/bin/node`), `node -v`(v22) 확인. → §glibc.
