...
**아직 측정되지 않은 것 (서명 화면이 필요하다)**: ① C1 참여자 귀속(구매사 화면에
PG 칸이 안 보이는가 — 계약 상세 응답에 `signature_fields` 가 없어 화면 외 근거가
없다) ② C2 좌표 원점이 T6(좌상단)과 같은가 ③ C3 `page_number` 1-based 착지
④ C4 강등된 참여자가 실제로 이메일 링크만으로 서명 화면에 닿는가. ①~③ 은 콘솔
초안 미리보기로 발송 없이 볼 수 있고, ④ 는 실발송이 필요하다. C4 의 위험 방향은
안전하다 — 강등 쪽이 되레 인증을 요구하면 우리 표시가 **과소**주장이 된다.
...
- **C6 실행(2026-08-08)**: 계약 `81057a54-43f3-4a75-8cfd-8b8a6e4779ba`(초안) — 취소 완료(HTTP 200).
  계약 `2c13dca0-37f9-4632-b97a-4a39c396532f`(초안) — **살아 있다.** 콘솔 미리보기로 C1~C3
  (참여자 귀속·좌표 원점·페이지 번호)을 볼 수 있게 남겨 뒀다. 확인 후 취소할 것.
  업로드 세션 2개(자연 소멸). 발송 0건.
