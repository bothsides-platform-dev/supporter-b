-- Greenfield schema (single initial migration). Hand-written per project
-- convention (no db:generate — snapshot chain intentionally not maintained).
-- Targets PG14+ DDL so it applies in PGlite (PG17), docker pg-test, and Supabase.
...
CREATE TYPE "public"."buyer_stage" AS ENUM('pending', 'negotiating', 'decided');--> statement-breakpoint

-- ===== TABLES (FKs added via ALTER below) =====
CREATE TABLE "users" (
...
CREATE TABLE "bids" (
	...
	"buyer_stage" "buyer_stage" DEFAULT 'pending' NOT NULL,
...
--> statement-breakpoint
CREATE TABLE "notifications_2026_05" PARTITION OF "notifications" FOR VALUES FROM ('2026-05-01 00:00:00+00') TO ('2026-06-01 00:00:00+00');--> statement-breakpoint
CREATE TABLE "notifications_2026_06" PARTITION OF "notifications" FOR VALUES FROM ('2026-06-01 00:00:00+00') TO ('2026-07-01 00:00:00+00');--> statement-breakpoint
CREATE TABLE "notifications_default" PARTITION OF "notifications" DEFAULT;--> statement-breakpoint
...
CREATE TRIGGER "trg_workspaces_updated_at" BEFORE UPDATE ON "workspaces" FOR EACH ROW EXECUTE FUNCTION set_updated_at();--> statement-breakpoint
CREATE TRIGGER "trg_workspace_invitations_updated_at" BEFORE UPDATE ON "workspace_invitations" FOR EACH ROW EXECUTE FUNCTION set_updated_at();--> statement-breakpoint
CREATE TRIGGER "trg_rfps_updated_at" BEFORE UPDATE ON "rfps" FOR EACH ROW EXECUTE FUNCTION set_updated_at();--> statement-breakpoint
CREATE TRIGGER "trg_rfp_invitations_updated_at" BEFORE UPDATE ON "rfp_invitations" FOR EACH ROW EXECUTE FUNCTION set_updated_at();--> statement-breakpoint
CREATE TRIGGER "trg_bids_updated_at" BEFORE UPDATE ON "bids" FOR EACH ROW EXECUTE FUNCTION set_updated_at();--> statement-breakpoint
CREATE TRIGGER "trg_bid_notes_updated_at" BEFORE UPDATE ON "bid_notes" FOR EACH ROW EXECUTE FUNCTION set_updated_at();
