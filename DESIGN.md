...
- **`countKind`** — 그 두 페이지는 `countKind="unread"` 를 넘긴다. 그러면 칩이 ① `count > 0` 에서 `primary`/`on-primary` 로 칠해지고(§7.3 하드룰), ② 스크린리더에 `미읽음 N건` 으로 읽히며(`sr-only` 라벨 + `.md-numeric` 숫자 span — `aria-label` 금지 이유는 §7.3), ③ `count === 0` 이면 중립톤으로 돌아간다(강조할 미읽음이 없다). **분기는 `PageHeader` 안에만 있다** — 호출부는 상수 prop 하나만 넘긴다. "0 이면 중립"을 페이지마다 다시 쓰면 두 페이지가 조용히 갈린다. `countKind` 를 넘기지 않는 나머지 호출부는 `'total'` 로 지금 모양 그대로다.
