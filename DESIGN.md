...
> **코치마크 스포트라이트 링 소프트 펄스**(`.coachmark-pulse`, `app/globals.css`): 온보딩 코치마크의 타깃 강조 링에 opacity 호흡 루프(1.8s ease-in-out infinite)를 더해 시선 유도를 보강한다. opacity 전용, `prefers-reduced-motion: reduce` 존중(저감 시 정지). 타깃 밖 클릭 시 1회성 유도 플래시(`.coachmark-nudge`)도 같은 원칙(opacity 전용 — 말풍선의 배치별 인라인 transform과 충돌하지 않도록 scale은 쓰지 않는다, reduced-motion 존중)을 따른다.
