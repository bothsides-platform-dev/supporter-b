...
**브라우저 계산값 쪽은 `e2e/landing-text-token-cascade.spec.ts`가 진다.** 소스 표기가 맞아도 캐스케이드에서 그 선언이 실제로 이겼는지는 fs-walk 유닛 가드가 볼 수 없다. 그 스펙이 무는 축은 셋이다 — ① 랜딩 컨트롤이 `text-sm`(14px)인지, ② 행간이 본문의 무단위 값을 그대로 물려받는지(`leading-[inherit]`), ③ **`app/globals.css`의 `body`가 정말 `--md-typescale-body-large-*`를 소비하는지**. ③은 v0.5.6.1에서 더했다: 그 선언을 리터럴(`font-size: 14px`)로 바꿔치기하면 본문 기준선이 조용히 되돌아가는데, 토큰 **선언**만 검사하는 유닛 가드는 소비처가 그 토큰을 쓰는지까지는 보지 않아 통과시킨다. 그래서 스펙은 기대값을 살아 있는 `body`에서만 파생하지 않고, 프로브 span으로 토큰 값을 직접 읽어 대조한다(`rem`은 루트 기준이라 `body`가 오염돼도 토큰 값이 나온다).
