...
### `sendFromTemplate`/`resolveStaleEmbedRef` 의 ref-clear 가 CAS 없이 블라인드 쓰기다 — 산 계약을 고아로 만들 수 있다 (P2, 컷 감사 다중 적발)
`contract-signing.ts`의 세 clear 분기(터미널 ref :847, 출처·판본 불일치 :875 — 이 릴리스 신규, 인증 미강제 :889)와 `resolveStaleEmbedRef`(:1234, 선존재)가 전부 `patchContract(active.id, { providerRef: null })` 를 **id 만으로** 건다. `patchContract`(리포지토리)는 `WHERE id = $id` 뿐 — `provider_ref` 현재값도 `status` 도 보지 않는다. 시나리오: A(`sendFromTemplate`)가 오래 걸리는 `getContract` 프로브를 기다리는 동안, 리스와 무관한 `attachProviderContract`(임베드 postMessage 경로 — 발송 리스를 안 잡는다)가 **같은 행에** 실제로 발송된 provider 계약을 바인딩하고 상태를 `sent` 로 올린다. A 의 프로브가 돌아와 clear 분기에 도달하면, 방금 바인딩된 **살아있고 이미 서명 요청 메일이 나간** provider_ref 를 그대로 지운다 — 그 계약은 우리 DB 에서 취소·다운로드·리마인드 어느 경로로도 다시는 손댈 수 없는 고아가 된다(삭제 API 없음, 복구는 수동 DB 개입뿐). 적대 리뷰와 보안 리뷰가 서로 다른 경로로 독립 적발해 교차 확인됨. 닫는 법: `clearDraftRefIf(id, expectedRef)` — `WHERE id=$id AND provider_ref=$expectedRef AND status='awaiting_pg_template'` CAS를 갖는 리포지토리 메서드를 신설해 네 clear 지점 전부에서 프로브 시점에 실제로 읽은 ref 를 조건으로 넘길 것(CAS 실패 = 그 사이 다른 경로가 진짜로 바인딩했다는 뜻이므로 지우지 말고 `CONTRACT_BUSY` 로 물러날 것). 847/889/1234 는 선존재(이 릴리스가 만든 게 아니다) — 875 만 신규지만 같은 취약 프리미티브를 재사용했다. (발견: dev→main 컷 감사 — adversarial + security, 교차 확인)

### `sendFromTemplate` 의 재사용 게이트가 검증한 ref 와 실제로 보내는 ref 가 다를 수 있다 (P2, 재확인·심화)
`isReusableTemplateDraft`(:1150)는 `findDraftRef` 로 그 순간 DB 를 다시 읽어 출처·판본을 검증하지만, **검증에 쓴 그 read 의 `providerRef` 를 버리고** 반환한다. 호출부(:958)가 실제로 보내는 값은 더 이전의 `active.providerRef`(:779 재조회 스냅샷)다 — 둘이 같은 객체라는 assert 가 없다. 위 P2(블라인드 clear)와 같은 경합 창(다른 경로가 발송 리스 밖에서 ref 를 갈아치우는 동안의 provider 프로브 왕복)에서, 게이트가 **DB 의 현재 ref(새 것)**를 검증해 통과시키고 코드는 **오래된 스냅샷(옛 것)**을 보내는 불일치가 가능하다 — 직접 추적 확인: `sendContract(providerRef)`(:1011)가 실제로 옛 문서를 발송(=서명 요청 메일 발신, 되돌릴 수 없음)한 **뒤에야** `markSentIfAwaiting` 의 `claimedAt` CAS(:1050)가 진다. CAS 실패 시 catch 블록(:1086)이 best-effort 로 그 ref 를 `snowsign.cancel` 하지만(:1111), 이는 **추가 서명을 막을 뿐 이미 나간 메일을 되돌리지 못한다** — "화면은 연결된 템플릿을 보냈다고 말하는데 실제로는 다른 문서가 서명 요청으로 나간다"는, 이 PR 전체가 막으려던 바로 그 결과다. (참고: 최초 리뷰는 이 CAS 가 "발송 전에" 막아 무해하다고 평가했으나, `sendContract` 호출이 CAS 보다 먼저라 그 평가는 틀렸다 — 직접 코드 추적으로 정정.) 이 결함은 **이 PR 이전부터 존재**했다(`active.providerRef` 를 갱신 없이 그대로 보내는 것 자체가 선행 코드 구조) — 새 게이트는 검증을 하나 더 추가했을 뿐 대상을 못 고쳤다. 닫는 법: `isReusableTemplateDraft` 가 검증한 `draft.providerRef` 를 반환하게 하고, 호출부가 `active.providerRef` 대신 그 값으로 send 하거나 최소한 `draft.providerRef === active.providerRef` 를 요구해 불일치 시 `CONTRACT_BUSY` 로 물러날 것 — 위 clear-CAS 항목과 같은 커밋에서 함께 고치는 것이 자연스럽다(둘 다 "프로브 이후 아무것도 재검증 없이 신뢰하지 않는다"는 같은 규율). (발견: dev→main 컷 감사 — adversarial 발견, security 도 독립 발견했으나 심각도 과소평가; 본인 직접 추적으로 재평가)

### 서명 관련 테스트 3건 — 재사용 게이트 판정 무근거·부수 취소 무단언·타입전용 불변식 (P4)
컷 감사(testing specialist)가 mutation-check 로 적발, 전부 latent(현재 관측된 실패 없음): ① `contract-signing.test.ts` 의 compose/구판본/레거시 재사용 테스트 셋이 `bindDraftRef` 시드 호출의 boolean 반환을 확인하지 않아(레거시 시드는 raw `db.update` 로 행 수도 미확인) 시드가 조용히 no-op 해도 전체 통과 — `expect(client.getContract).toHaveBeenCalledWith(...)` 로 프로브 도달을 단언할 것. ② 출처 불일치 분기(:872)는 "provider draft 를 취소하지 않는다"는 불변식을 주석으로 문서화했지만 인접 테스트의 `client.cancel` 이 무단언 `vi.fn()` 이라 이 동작이 반대(터미널 ref 분기, 취소함)와 한 줄 스왑돼도 안 걸린다 — `expect(client.cancel).not.toHaveBeenCalled()` 를 추가할 것. ③ `SigningContractPatch.providerRef?: null` 좁히기(타입 레벨 원자성 보장)를 재는 런타임 드리프트 가드가 없어 `as any` 하나로 조용히 되돌아갈 수 있다 — `@ts-expect-error` 고정 테스트나 `patchContract` 의 런타임 거부를 추가할 것. (발견: dev→main 컷 감사 — testing specialist)
...
### 템플릿 바인딩 이후 임베드로 발송되면 출처 컬럼이 거짓말을 남긴다 (P4, 오늘 무해)
어떤 행이 템플릿 경로로 바인딩됐다가(`origin='template'`, `snowsign_template_id=V`) 그 초안이 버려지고 결국 건별 임베드로 발송되면, `bindDispatchedContract`/`markSentIfAwaiting` 은 새 `provider_ref` 만 쓰고 `provider_draft_origin`·`snowsign_template_id` 는 손대지 않는다 — 그 행은 영원히 "출처: template, 판본: V" 라고 말하지만 실제 provider_ref 는 그것과 무관한 임베드 계약이다. 오늘은 무해하다(`findDraftRef` 가 `provider_ref` 로 먼저 키를 잡고, 상태 게이트가 재사용을 막고, `stripProviderRefs` 가 구매사 페이로드에서 걷어낸다) — 다만 이 컬럼들을 "이 초안이 누구 것인가"의 근거로 쓰는 것이 이 PR 의 요점이라, Stage 2 가 그대로 물려받으면 거짓 전제 위에 서게 된다. 닫는 법: `markSentIfAwaiting`/`bindDispatchedContract` 가 provider_ref 를 새로 쓸 때 origin·판본도 함께 지우거나 갱신할 것. (발견: dev→main 컷 감사 — adversarial)
