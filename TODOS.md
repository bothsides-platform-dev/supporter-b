...
건별 임베드는 계약을 브라우저 안에서 만든다 — 완료 postMessage 가 유실되면 **계약은 실제로 발송됐는데 딜룸은 awaiting 에 갇힌다**(서명 링크는 양측에 이미 나갔다). ~~자동 매칭 복구는 불가능하다 — `external_id` 가 회신되지 않아 어느 계약이 우리 딜의 것인지 식별할 수 없다.~~ — **이 결론은 틀렸다. 해결 (v0.4.37.0)**: `external_id` 를 유일한 상관키로 전제한 탓이었다. 실제로는 `GET /v1/contracts/{id}` 가 **참여자 이메일을 회신하고**, 우리는 구매사 서명 담당자 이메일을 안다(임베드 패널이 PG 에게 띄워 주는 값이고 `participantMismatch` 가 이미 같은 대조를 한다). `recoverOrphanedSend` 가 `GET /v1/contracts` 목록 → 미바인딩 후보 → 상세의 참여자 이메일 일치로 좁히고, **정확히 하나일 때만** 채택한다(단일 org 키라 다른 테넌트 계약도 섞이므로 애매하면 기권 — 잘못 붙이면 남의 계약이 이 딜룸에 노출된다). 채택은 `attachProviderContract` 에 위임해 상태·바인딩 유일성·CAS 게이트를 재사용한다. `poll-signing-status` cron 이 10분 이상 대기한 계약에 대해 재넛지보다 **먼저** 돌린다. 남은 것은 수동 복구 입력(후보 0개거나 모호할 때의 최후 수단)뿐이다. (발견: 건별 임베드 전환 설계, v0.4.37.0)
