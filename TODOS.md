...
### 설정 사업자번호 "현재와 동일" 판정이 표기 차이로 어긋난다 (P4)
`WorkspaceBizNoForm` 은 조회 결과(`124-81-00998`, 하이픈)와 저장값을 문자열 그대로 비교한다. 실제 가입으로 저장된 값은 숫자만(`1248100998`)이라 같은 번호를 다시 조회해도 "현재 사업자번호와 동일합니다" 가 뜨지 않고 저장 버튼이 켜진다(저장해도 값은 같다 — 불필요한 국세청 재조회 1회). 시드 데이터는 하이픈 형식이라 기존 테스트가 이 경우를 못 본다. 수정: 비교 전 양쪽을 숫자로 정규화. (발견: /qa 구매사 가입 2026-09-23)
