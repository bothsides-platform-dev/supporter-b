...
### 짙은 컨테이너 위 보조 텍스트가 AA 미달 (P2)
`on-surface-variant`(#6B7079) 는 `surface`(#FBFBFC) 위에서 4.81:1 이라 본문 AA 를 통과하지만, 배경이 짙어지면 **`surface-container` 4.44:1 · `surface-dim` 4.25:1 · `surface-container-high` 4.21:1 · `surface-container-highest` 3.95:1 · 각 `*-container` 3.85~4.29:1** 로 4.5:1 아래로 떨어진다. v0.4.11.0 의 `outline` 스윕은 모든 지점을 개선했지만(1.4:1 → 4.2:1 이상) 이 계층까지 AA 로 끌어올리지는 못했다. 실사례: `WizardStepSidebar.tsx:71`(9px 볼드 스텝 번호 on container-high), `bid-detail/BidPdfPane.tsx:23`(container-high 패널 안 안내문), `RfpAttachmentDropzone.tsx:160`(드래그 중 container-high 로 전환), `OpportunityList.tsx:21`(container-high 칩, 선존재). 드리프트 가드는 토큰 *이름*만 검사하므로 이 축은 잡지 못한다. 선택지: ① `on-surface-variant` 를 한 단계 더 어둡게(약 483곳 영향 + 다크 모드 동반 조정), ② 표면 계층별 보조 토큰 쌍 도입, ③ 짙은 컨테이너 위에서는 `on-surface` 로 올리는 규칙. DESIGN.md §2 에 현재 한계가 명시돼 있다. (발견: /ship 적대 리뷰 F13, 2026-07-22, v0.4.11.0)

### 디자인 하드룰 가드 커버리지 확장 (P3)
`lib/design/__tests__/{outline-text,mono-label}-drift.test.ts` 는 라인 스코프 문자열 스캔이라 구조적 사각지대가 있다. 적대 리뷰가 실증한 것들: ① **CSS 미스캔** — `walk()` 가 `.ts`/`.tsx` 만 훑어 `app/globals.css` 에 `.foo { color: var(--md-sys-color-outline) }` 유틸을 만들면 82곳 문제가 무가드로 재발한다(`styles/tokens.css` 는 스캔 루트 밖). ② **dead-variant 가드가 한 줄 안에서만 성립** — 여러 줄 `cn()`, `cva()` variants, `clsx` 객체 문법, 공용 클래스 상수, 토큰 아닌 색(`text-muted-foreground`)은 전부 안 보인다. 실재 사례: `components/landing/LandingNav.tsx:53` 의 `group-data-[over-dark]/lheader:hover:text-[...]` 는 공용 상수 + 템플릿 리터럴 합성이라 두 겹으로 사각이다. ③ **dead-variant 가 3개 표기 중 1개만 커버** — `TEXT_COLOR_SPELLINGS` 가 나열한 v4 shorthand·`[color:var()]` 표기는 dead-variant 쪽엔 미적용. 근본 해법은 라인이 아니라 **합성(composition) 단위 분석**이며, 그 전까지는 알려진 한계로 각 테스트 파일 상단에 명시해 두었다. (발견: /ship 적대 리뷰 F3~F6, 2026-07-22, v0.4.11.0)
...
### 밑줄 인풋 클래스 문자열이 15곳에 복제됨 (P3)
`components/forms/inputs.tsx` 의 `underlineInputClass` 가 공용 상수로 존재하는데도 거의 동일한 문자열이 14개 파일에 복붙돼 있다(`app/(public)/{login,invite}`·`signup/{buyer,pg}/**`·`password/forgot`·`components/auth/{PasswordField,PhoneVerificationField,BuyerWorkspaceForm}`·`components/rfp/BizLookupField`·`components/settings/InviteMemberForm`·`components/workspace/CreateWorkspaceForm`·`components/pending-approval/EmailVerifySection`). v0.4.11.0 의 대비 스윕에서 `placeholder:` 색 하나를 바꾸는 데 15곳을 손대야 했던 것이 비용 증거다. 복제본마다 미세한 차이가 있어(`md-numeric`·`pr-10`·`disabled:opacity-50`·보더 유무·`tracking`) 단순 치환은 위험 — 공용 상수를 옵션 인자로 확장하고 각 사용처를 하나씩 이관해야 한다. 대비 PR 에서는 범위 밖으로 두었다. (발견: /ship maintainability 리뷰 2026-07-22, v0.4.11.0)

### 3차 텍스트 톤 소멸에 따른 위계 재설계 — 육안 확인 필요 (P3)
v0.4.11.0 이 `outline` 을 텍스트에서 걷어내면서 텍스트 색이 2단(`on-surface`/`on-surface-variant`)으로 줄었다. AA 를 통과하면서 `on-surface-variant` 보다 옅은 색은 만들 수 없으므로(상한 L≤0.175 vs 실제 L=0.161) 그 아래 위계는 타입스케일로 만들어야 하는데, 스윕은 색만 올렸고 크기·굵기는 손대지 않았다. 명시적으로 접은 두 곳(`WizardStepSidebar` 라벨·`PgProcessStepRail` 제목 — 배지·도트가 상태를 대신 진다) 외에 리뷰가 지목한 잔여 후보: ① `components/landing/CostComparisonChart.tsx:25` 이어브로우와 단위 라벨이 같은 색·크기·서체가 됨, ② `BidStepProposal.tsx:57`·`RfpAttachmentDropzone.tsx:160` 의 "업로드 지시문 / 용량 힌트" 두 단이 한 톤으로 붙음, ③ `ProblemCard.tsx:13` 의 clamp(28–44px) 장식 숫자가 워터마크에서 읽히는 2차 요소로 바뀜(대형 텍스트라 AA 기준은 3:1 이므로 opacity 로 되돌릴 여지 있음). 전부 신뢰도 3–4 의 육안 판단 건이라 `/design-review` 로 실제 화면을 보고 결정한다. (발견: /ship design 리뷰 2026-07-22, v0.4.11.0)
