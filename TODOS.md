...
v0.4.43.0 의 템플릿 수정은 잠금·버전 대조가 없다 — 두 담당자가 같은 템플릿을 열어 각자 저장하면 나중 저장이 provider 템플릿 id 를 덮고 먼저 것은 무해한 고아가 된다(데이터 오손은 없고 한쪽 편집이 사라질 뿐). 수정 중 rename 도 같은 결말(에디터 저장이 name 을 함께 쓴다). 발송 리스 같은 CAS 를 붙이려면 행에 갱신 카운터가 필요한데, 템플릿 편집 빈도상 과설계라 수용한다 — 실사용에서 충돌 보고가 나오면 재평가. (발견: 기능 설계 2026-08-05)
