...
건별 임베드는 계약을 브라우저 안에서 만든다 — 완료 postMessage 가 유실되면 **계약은 실제로 발송됐는데 딜룸은 awaiting 에 갇힌다**(서명 링크는 양측에 이미 나갔다). 지금은 PG 가 임베드를 다시 열어 두 번째 계약을 만드는 것 말고 방법이 없다.

**설계는 확정됐다(v0.4.37.0 조사). 구현만 남았다 — 별도 PR.** ~~`external_id` 가 회신되지 않으니 자동 매칭은 불가능하다~~ 는 틀린 결론이었다(그 필드를 유일한 상관키로 전제한 탓). 실제 상관키는 **참여자 이메일**이다: `GET /v1/contracts/{id}` 가 참여자를 이메일까지 회신하고, 우리는 구매사 담당자와 낙찰 PG 담당자 이메일을 둘 다 안다.

구현 시 반드시 지킬 것 — 이번에 프로토타입을 만들었다가 적대 리뷰에서 아래가 전부 드러나 통째로 빼냈다:
1. **상관키는 구매사 이메일 하나로는 안 된다.** 한 담당자가 견적을 여럿 내는 건 평범해서, 그것만 보면 대기 중인 딜이 **다른 딜의 계약**을 가져간다(PG-A 딜룸에 PG-B 계약이 붙어 취소권·완료본이 경쟁사에게 넘어간다). 낙찰 PG 담당자 이메일도 참여자에 있어야 한다. 같은 (구매사, PG) 쌍의 딜이 둘 이상 동시에 대기 중인 경우는 여전히 모호하므로 그때도 기권해야 한다.
2. **호출 예산을 설계해야 한다.** rate limit 은 분당 100회이고 그 키를 모든 PG사·모든 서명 기능이 공유한다. 후보마다 상세 조회를 돌리면 cron 한 틱이 예산을 수십 배 넘기고, 429 재시도가 증폭시켜 `attachProviderContract` 부터 실패한다 — **고아가 더 생겨 복구 부하가 더 커지는 양성 피드백**이다. 목록 1회를 여러 행이 공유하고 상세 조회 총량에 상한을 둘 것.
3. **스케줄 마커가 필요하다.** `lastPolledAt` 은 이미 폴링·재넛지(7일)가 겹쳐 쓰고 있어, 복구까지 얹으면 세 일정이 한 컬럼을 공유해 서로를 조용히 깨뜨린다. 그리고 마커 없이 `findStaleAwaiting` 을 그대로 쓰면 **PG 가 아직 안 올린 평범한 딜들이 목록 앞을 영구 점유**해, 진짜 고아가 영영 검사되지 않는다.
4. **페이지네이션.** "후보가 정확히 하나"라는 안전 논거는 1페이지 안에서만 참이다. 잘린 결과에서는 기권해야 한다.
5. **감사 로그**를 사람이 보낸 것처럼 남기면 안 된다(cron 채택인데 `signing.sent` + 실명 actor).
6. 진행 중인 임베드 세션(리스 살아 있음)은 건너뛸 것 — 아니면 작업 중인 PG 의 패널이 닫힌다.

더 안전한 대안: 자동 채택 대신 후보를 PG 에게 보여주고 확인받는 UI. (발견: 건별 임베드 전환 설계, v0.4.37.0)
