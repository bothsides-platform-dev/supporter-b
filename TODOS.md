...
v0.4.42.0 이 contract_id 화이트리스트 + 재조회 리미터(30/분)로 **증폭 축은 닫았지만**, 같은 (body, signature) 쌍의 재전송 자체는 여전히 유효하다(타임스탬프/nonce 없음 — trigger-only 설계라 상태 부작용은 0). Public API 문서에 웹훅 절이 없어 provider 가 타임스탬프 헤더를 보내는지부터 실측이 필요하다(콘솔 전송 로그 또는 수신 헤더 덤프 1회). 보내면 허용창 검증 추가, 안 보내면 수용 리스크로 못박는다. (발견: 업계 표준 감사 2026-08-05)
