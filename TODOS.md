...
### 가입 약관·개인정보·마케팅 동의가 서버에 기록되지 않는다 (P1)
가입 1단계(`app/(public)/signup/{buyer,pg}/page.tsx`)는 필수 동의 없이는 진행을 막지만, 동의 사실은 sessionStorage draft 의 `agreedAt` 에만 남고 `finalizeSignup` → `signupCompleteAction` 어디에도 실리지 않는다. 마케팅 수신 동의(선택)는 draft 에조차 저장되지 않는다. 결과: 어떤 사용자가 언제 어떤 판의 약관에 동의했는지, 마케팅 수신에 동의했는지 DB 에 증거가 없다(`users`·`workspace_members` 에 해당 컬럼 없음). 광고성 정보 발송을 시작하는 순간 수신 동의 증빙이 필요해지고, 약관 개정 시 재동의 대상도 가릴 수 없다. 수정: 동의 시각·약관 판·마케팅 여부를 가입 액션 입력에 싣고 서버가 기록(DDL 선행 배포). 초대·canonical PG 합류 경로도 같은 1단계를 지나므로 네 액션 모두. (발견: /qa 구매사 가입 2026-09-23)

### 신규 구매사가 가입 중 입력한 `next` 목적지를 승인 후 잃는다 (P4)
랜딩 CTA(`/rfp-create`) → 로그인 → 가입으로 들어오면 `next` 가 draft 에 실려 가입 직후 `/rfp-create` 로 보내지만, 워크스페이스가 pending 이라 곧바로 `/pending-approval` 로 튕기고 승인 후에는 `/home` 으로 들어온다. 첫 진입의 의도(견적 요청)가 한 번 더 클릭으로 밀린다. 홈의 `견적 요청하기` CTA 가 바로 보여 실해악은 낮다. (발견: /qa 구매사 가입 2026-09-23)

### PG 가입 1·2단계도 뒤로가기 시 입력이 사라진다 (P3)
구매사 가입은 draft 로 1·2단계를 복원하도록 고쳤지만(/qa 2026-09-23, `fix/qa-buyer-signup`), PG 가입 `app/(public)/signup/pg/page.tsx`·`workspace/PgWorkspaceStep.tsx` 는 여전히 빈 폼으로 시작한다. 1단계는 구매사와 같은 방식(`workspaceType === 'pg'` draft 복원)으로 바로 옮길 수 있고, 2단계는 canonical PG 선택·직접 입력 분기를 함께 복원해야 해서 따로 본다.

### ~~읽기 전용 화면이 사업자번호를 하이픈 없이 보여준다 (P4)~~ — 해결 (`fix/qa-buyer-signup`)
가입 조회칸은 `124-81-00998` 인데 저장값은 숫자만이라 견적 요청 1·4단계, PG 브리프, 요청 조건, 설정 프로필이 `1248100998` 로 보였다. `formatBizNoDisplay`(`lib/utils/format.ts`)로 다섯 곳을 맞췄다. (발견: /qa 구매사 가입 2026-09-23)

### ~~설정 사업자번호 "현재와 동일" 판정이 표기 차이로 어긋난다 (P4)~~ — 해결 (`fix/qa-buyer-signup`)
`WorkspaceBizNoForm` 이 조회 결과(하이픈)와 저장값(숫자만)을 문자열 그대로 비교해 같은 번호를 다시 조회해도 "현재 사업자번호와 동일합니다" 가 뜨지 않고 저장이 열렸다. 표기를 맞춘 뒤로는 두 값이 똑같이 보여 더 헷갈렸다 — 양쪽을 숫자로 맞춰 비교한다. (발견: /qa 구매사 가입 2026-09-23)

### 가입 1단계가 다른 가입 경로의 draft 키를 그대로 이어받는다 (P3, 선존재)
구매사 가입 1단계 제출은 `writeSignupDraft({ ...draft, email, password, ... })` 로 기존 draft 를 통째로 합친다. 같은 탭에서 워크스페이스 초대 링크나 주요 PG사 선택을 거쳤다면 `wsInviteToken`·`selectedPgWorkspaceId` 가 남고, `finalizeSignup` 은 그 두 키를 `workspaceType` 보다 먼저 보므로 구매사로 가입하려던 사용자가 초대/PG 합류 경로로 새어 나간다. 이메일을 바꿔 다시 진행해도 이전 사람의 `name`·`phone`·`phoneVerificationId` 가 남는다. 수정: 구매사 1단계 제출 시 다른 경로 전용 키를 지우고, 이메일이 바뀌면 이후 단계 값도 비운다. (발견: /ship 적대적 리뷰 2026-09-23)
...
- ~~**목록에 상한이 없다.** `ARCHIVE_UPLOAD_CAP_PER_WORKSPACE`(200)는 `source='upload'` 만 센다 — 서명 출처 행은 완료 계약당 2행씩 **무제한**이고 `listByWorkspace` 에 `.limit()` 이 없다.~~ — 해결 (v0.18.0.0): `listPageByWorkspace` 가 `coalesce(contracted_at, created_at)`+`id` 키셋 커서로 50건씩 끊고, 검색(제목·상대방)은 페이지가 아니라 **전체 보관함**에 SQL 로 적용된다(LIKE 메타문자 이스케이프 포함). 페이지 크기는 서버가 정하고 커서·검색어는 액션의 zod 게이트를 지난다. `listByWorkspace` 는 내부 호출자용으로 남는다.
- **겹쳐 도는 회차가 같은 계약을 이중으로 공급자에 요청한다 (P2, v0.18.0.0).** `claimSigningAttempt` CAS 는 **같은 attempts 값**을 두 worker 가 잡는 것만 막는다 — 앞 회차가 이미 값을 전진시켜 두었으므로, 겹쳐 시작한 뒤 회차는 같은 계약을 **정상 claim** 해 동시에 provider I/O 를 한다. 배치 3건 x (15s+15s) 타임아웃이 2분 cron 간격을 넘길 수 있어 실제로 겹친다. 이번 컷은 그 겹침이 **남의 완료본을 지우는** 축만 닫았다(전이 성공에만 삭제 — `markSigningFailed` 가 boolean 을 돌려준다). 남은 피해는 공급자 호출 중복과 `MAX_HYDRATE_ATTEMPTS=10` 이 실질 5회로 줄어드는 것. 닫는 법: `last_attempt_at` 기반 리스(발송 리스 `claimForSend` 전례)로 in-flight 소유권을 표현하거나, cron 스텝에 실행 중복 가드를 둔다.
- **보관함 검색에서 견적번호·초성이 빠졌다 (P3, v0.18.0.0).** 검색을 서버 `ILIKE` 로 옮기면서 대상이 제목·상대방 두 칼럼이 됐다 — 예전 클라이언트 필터(`lib/contract-archive/search.ts` 의 `matchesQuery`)는 견적번호도 훑고 `es-hangul` 초성 매칭도 했다. 목록 행에 견적번호가 **보이는데** 그것으로 검색하면 0건이라 특히 헷갈린다. 그 모듈은 이제 **프로덕션 호출자가 없고**(자기 테스트만 남아 초록이라 "커버됨"으로 읽힌다) 삭제하거나 은퇴 표시해야 한다. 닫는 법: `rfp_code` 를 `ILIKE` 대상에 더하고, 초성은 저장 시 초성 칼럼을 계산하거나 포기한다고 명시한다.
