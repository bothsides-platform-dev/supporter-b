...
### e2e 가 dev 서버를 상대로 돈다 — 콜드 컴파일이 타임아웃 상수로 새어 나온다 (P3)
`playwright.config.ts` 의 webServer 가 `pnpm dev` 라, 각 라우트의 **첫 방문이 곧 Turbopack 컴파일**이다. 그래서 스펙 곳곳에 45s·60s·120s 같은 여유 상수가 박혀 있고(로그인 대기, 인터셉트 모달 대기 등), 그 값들은 회귀를 막는 게 아니라 **컴파일 시간을 덮는 패딩**이다. 실제로 2026-08-26 수리에서 이 성질이 드러났다 — 앞선 두 스펙이 3분씩 타임아웃하며 라우트를 우연히 데워 주던 것이 사라지자, **그동안 초록이던 딜룸 모달 스펙 2건이 CI 에서 처음으로 빨개졌다**(로컬은 초록, URL 은 이미 바뀌었고 모달만 5초를 넘김). 즉 이 하네스에서는 "초록"이 실행 순서에 의존한다. 닫는 법: CI e2e 를 `next build && next start` 로 돌린다(빌드 2-4분이 붙지만 전 스펙의 콜드 컴파일이 사라져 총시간은 비슷하고, 타임아웃 상수를 실제 회귀 임계로 되돌릴 수 있다). 로컬은 `pnpm dev` 유지가 편하므로 `process.env.CI` 로 갈라야 한다. (발견: e2e 수리 2026-08-26)

### PG 칩에 선택 상태가 노출되지 않는다 — 스크린리더도 테스트도 못 읽는다 (P3)
`RfpStep3PgSelect.tsx:88-113` 의 PG 칩은 선택 여부를 **CSS 클래스로만** 알린다 — `aria-pressed` 도 `data-*` 도 없다. 결과가 둘이다: ① 스크린리더 사용자는 어떤 PG 가 선택됐는지 알 수 없고 ② e2e 는 `N개 선택됨` 문구를 유일한 관측점으로 삼아야 한다(2026-08-26 수리에서 실제로 그렇게 했다). 같은 파일의 견적 유형 버튼은 이미 `aria-pressed` 를 쓴다(`RfpStep2Content:84`) — 관례가 있는데 여기만 빠졌다. 닫는 법: 칩에 `aria-pressed={selected}` 를 더한다(시각 변화 0). 곁들여 **확인이 필요한 제품 결정**: `RfpCreateWizard:74-80` 이 첫 진입에서 사용 가능한 PG 를 **전부** 자동 선택한다 — 기본값이 "전원 발송"이라는 뜻이고, 봉인 입찰 제품에서 의도된 기본인지 확인할 것. (발견: e2e 수리 2026-08-26)
