...
건별 임베드는 계약을 브라우저 안에서 만든다 — 완료 postMessage 가 유실되면 **계약은 실제로 발송됐는데 딜룸은 awaiting 에 갇힌다**(서명 링크는 양측에 이미 나갔다). 지금은 리스(30분)가 풀린 뒤 PG 가 임베드를 다시 열어 두 번째 계약을 만드는 것 말고 방법이 없다. 재료 절반은 있다 — `attachProviderContract` 가 멱등이라 다시 붙이는 것 자체는 안전하다. 계약을 **찾는** 쪽(`GET /v1/contracts` 래퍼)은 없다: 호출자 없는 죽은 코드가 되어 YAGNI 로 걷어냈으니, 복구 경로를 지을 때 같이 되살린다. **실측으로 갈림길이 정해졌다**: `external_id` 가 회신되지 않으므로(SNOWSIGN_SANDBOX Q3) `GET /v1/contracts` 를 훑어도 어느 계약이 우리 딜의 것인지 식별할 수 없다 — **자동 매칭 복구는 불가능하다.** 남은 수단은 ① 리스 만료된 awaiting 에서만 노출되는 최소 복구 입력(계약 ID 를 사람이 넣는다), ② 스노우싸인에 `external_id` 회신/필터 요청. 완화 요소: 완료 postMessage 는 실측에서 안정적으로 도착했고(유실은 탭이 죽는 등의 경우), 계약이 미결로 남아도 구매사에게는 서명 요청이 이미 가 있어 서명 자체는 진행된다 — 다만 우리 딜룸이 그 사실을 모른다. (발견: 건별 임베드 전환 설계, v0.4.37.0)
