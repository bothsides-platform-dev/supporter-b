...
구매사 가입은 draft 로 1·2단계를 복원하도록 고쳤지만(v0.22.5.0), PG 가입 `app/(public)/signup/pg/page.tsx`·`workspace/PgWorkspaceStep.tsx` 는 여전히 빈 폼으로 시작한다. 1단계는 구매사와 같은 방식(`workspaceType === 'pg'` draft 복원)으로 바로 옮길 수 있고, 2단계는 canonical PG 선택·직접 입력 분기를 함께 복원해야 해서 따로 본다.

### ~~읽기 전용 화면이 사업자번호를 하이픈 없이 보여준다 (P4)~~ — 해결 (v0.22.5.0)
...
### ~~설정 사업자번호 "현재와 동일" 판정이 표기 차이로 어긋난다 (P4)~~ — 해결 (v0.22.5.0)
