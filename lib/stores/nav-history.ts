 ...
 * `sync(url, viaPop)` on every route change; the Breadcrumb calls
 * `markBack/markForward` (its own button intent) right before
 * `router.back()/forward()`.
 *
 * `viaPop` tells a back/forward navigation (browser buttons or our own, which
 * fire `popstate`) apart from a push (links/sidebar/chord, which don't). Without
 * it a forward link to the page you just left looks identical to a browser back.
 */
type NavHistoryStore = {
  ...
  pendingDir: -1 | 0 | 1;
  sync: (url: string, viaPop?: boolean) => void;
  markBack: () => void;
  markForward: () => void;
  reset: () => void;
};
...
export const useNavHistoryStore = create<NavHistoryStore>((set) => ({
  ...
  sync: (url, viaPop = false) =>
    set((s) => {
      ...
      if (viaPop) {
        // Browser back/forward (no hint): locate the entry by matching a neighbor.
        if (url === s.entries[s.index - 1]) return { index: s.index - 1 };
        if (url === s.entries[s.index + 1]) return { index: s.index + 1 };
      }
      // A push: drop any forward branch and append.
