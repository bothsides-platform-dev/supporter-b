 ...
 * useCentrifugoSubscription) and surfaces two full-IM signals: incoming
 * messages/read-receipts (server publishes) and typing indicators (client
 * ephemeral publishes).
 *
 * Online presence is sourced from useWorkspacePresence, not this hook.
 *
 ...
 * `{ typingUserIds:[], sendTyping:no-op, connected:null }` and the thread runs
 * entirely off its static loader. The live path activates only when
 ...
 *
 * Typing + sendTyping stay here (channel-specific); only the
...
export function useChatChannel(
  ...
  return { typingUserIds, sendTyping, connected };
}
