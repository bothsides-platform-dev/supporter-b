...
export interface WorkspaceRepo {
  ...
  listPgRecommendationGroups(opts?: { includeTest?: boolean }, tx?: Tx): Promise<{ id: string; name: string; mccCode?: string; pgWorkspaceIds: string[] }[]>;
  /** 워크스페이스 + 멤버 동기화. */
...
export interface BidRepo {
  ...
  /** 구매사 목록용 제출 견적 건수만 집계한다. 없는 RFP는 Map에서 빠진다. */
  countSubmittedByRfpIds(rfpIds: string[], tx?: Tx): Promise<Map<string, number>>;
  /** 한 PG 워크스페이스의 모든 입찰. */
