  ...
  it('모든 active 워크스페이스를 role:admin, unreadCount:0으로 반환한다', async () => {
    ...
    expect(list).toEqual(
      expect.arrayContaining([
        { id: buyer.id, name: '구매사A', type: 'buyer', role: 'admin', status: 'active', unreadCount: 0, hasLogo: false },
        { id: pg.id, name: 'PG-B', type: 'pg', role: 'admin', status: 'active', unreadCount: 0, hasLogo: false },
      ]),
    );
  });
