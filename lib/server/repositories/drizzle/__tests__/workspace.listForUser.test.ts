  ...
  it('returns every workspace the user belongs to with id, name, type, role', async () => {
    ...
    expect(list).toEqual(
      expect.arrayContaining([
        { id: wsBuyer.id, name: '구매사A', type: 'buyer', role: 'admin', status: 'active', unreadCount: 0, logoUpdatedAt: null, isDemo: false, memberApprovalStatus: 'approved' },
        { id: wsPg.id, name: 'PG-B', type: 'pg', role: 'member', status: 'active', unreadCount: 0, logoUpdatedAt: null, isDemo: false, memberApprovalStatus: 'approved' },
      ]),
    );
  });
