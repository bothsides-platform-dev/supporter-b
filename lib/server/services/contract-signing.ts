...
export class ContractSigningService {
  ...
  /**
   * cron 배치 — 대기에 오래 머문 계약들에 고아 복구를 시도한다.
   *
   * 기본 10분은 리스(5분)보다 넉넉히 길다. **작업 중인 임베드 세션을 고아로 오인하지
   * 않기 위해서다** — 그 시점에는 아직 계약이 만들어지지도 않았을 수 있다.
   *
   * 재넛지(7일)보다 훨씬 짧게 도는 이유: 고아는 즉시 갇히고, 그동안 구매사는 서명
   * 요청 메일을 이미 받은 채 딜룸에서는 아무 진행도 못 본다.
   */
  async recoverStaleOrphans(
    olderThanMs = 10 * 60_000,
    limit = 10,
  ): Promise<{ recovered: number }> {
    const before = new Date(Date.now() - olderThanMs);
    const stale = await this.signingRepo.findStaleAwaiting(before, limit);
    let recovered = 0;
    for (const c of stale) {
      const r = await this.recoverOrphanedSend(c.id);
      if (r.ok && r.recovered) recovered += 1;
    }
    return { recovered };
  }

  /**
   * 고아 복구 — 발송은 실제로 됐는데 완료 postMessage 가 유실돼 대기에 갇힌 계약을
   * 되찾는다(양측에는 서명 요청 메일이 이미 가 있다).
   *
   * `external_id` 로는 못 찾는다(실측 Q3 — 회신되지 않음). 대신 **참여자 이메일**이
   * 상관키다: 임베드가 참여자 프리필을 지원하지 않아 PG 가 구매사 담당자 주소를 직접
   * 타이핑하고, 우리는 그 주소를 안다(패널이 띄워 주는 값이고 `participantMismatch` 가
   * 이미 같은 대조를 한다).
   *
   * **애매하면 붙이지 않는다.** 단일 org 키라 다른 테넌트의 계약도 목록에 섞이므로,
   * 후보가 정확히 하나일 때만 채택한다. 잘못 붙이면 남의 계약 상태와 완료본이 이
   * 딜룸에 노출된다 — 못 찾는 것보다 훨씬 나쁘다.
   *
   * 채택은 `attachProviderContract` 에 그대로 위임한다. 상태·소유·바인딩 유일성·CAS
   * 게이트와 알림 팬아웃을 한 번 더 쓰지 않고 재사용하기 위해서다(복제하면 드리프트한다).
   */
  async recoverOrphanedSend(
    signingContractId: string,
  ): Promise<ServiceResult<{ recovered: boolean }>> {
    const found = await this.signingRepo.findById(signingContractId);
    if (!found) return { ok: false, error: 'CONTRACT_NOT_FOUND' };
    const { contract } = found;
    if (contract.status !== 'awaiting_pg_template') return { ok: true, recovered: false };

    const rfp = await this.rfpRepo.findById(contract.rfpId);
    if (!rfp?.awardedBidId) return { ok: true, recovered: false };
    const bid = await this.bidRepo.findById(rfp.awardedBidId);
    ...
    let candidates;
    try {
      candidates = await this.snowsign.listContracts({ status: 'pending', perPage: 100 });
    } catch (e) {
      // 복구는 보조 경로다 — 실패해도 조용히 넘어간다(다음 cron 이 다시 시도한다).
      logger.warn('signing.recover_list_failed', { contractId: contract.id, err: String(e) });
      return { ok: true, recovered: false };
    }

    const matches: string[] = [];
    for (const c of candidates) {
      // 이미 다른 계약 행이 쥔 건 후보가 아니다(선착순 제약과 같은 규칙).
      const bound = await this.signingRepo.findByProviderRef(c.contractId);
      if (bound) continue;
      let detail;
      try {
        detail = await this.snowsign.getContract(c.contractId);
      } catch {
        continue;
      }
    ...
    const providerRef = matches[0]!;
    // 낙찰 PG 를 행위자로 세운다 — 실제로 그 계약을 보낸 주체이고, attach 의 ACL 이
    // 그대로 통과해야 게이트를 재사용할 수 있다.
    const r = await this.attachProviderContract(contract.rfpId, providerRef, {
      userId: bid.submittedBy,
      workspaceId: bid.pgWsId,
    });
    if (!r.ok) {
      logger.warn('signing.recover_attach_failed', {
        contractId: contract.id,
        providerRef,
        err: r.error,
      });
    ...
    logger.info('signing.recovered', { contractId: contract.id, providerRef });
