...
/**
 * 선정 후 전자서명 요청 안내 메일 — 공통 합의서와 기존 계약 양쪽에 적용한다.
 *
...
export function SigningAwaitingTemplate({
  ...
  return (
    <Layout
      preheader={`${rfpId} 계약 내용을 확인하고 전자서명을 요청해 주세요.`}
      serial={`견적 요청 / ${rfpId}`}
    >
      <h1
        style={{
      ...
      <p style={{ margin: '0 0 16px', fontSize: '14px' }}>
        {isNudge
          ? '선정된 견적의 계약 진행 상태를 확인해 주세요. 딜룸의 계약 탭에서 내용을 확인하고 전자서명을 요청할 수 있어요.'
          : '보내신 견적이 선정됐어요. 딜룸의 계약 탭에서 내용을 확인하고 전자서명을 요청해 주세요.'}
      </p>
      ...
      {isNudge ? (
        <p style={{ fontSize: '13px', color: '#555', margin: '24px 0 0' }}>
          이미 요청했다면 딜룸의 계약 탭에서 발송 결과를 확인해 주세요.
        </p>
      ) : null}
    </Layout>
  );
