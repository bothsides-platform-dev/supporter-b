...
export const SIGNING_ERROR_MESSAGES: Record<string, string> = {
  ...
  // 서식 종류가 경로와 어긋났다(조항형을 PDF 경로로, 또는 그 반대). 화면이 종류에
  // 따라 갈라 보내므로 정상 경로로는 안 나오지만, 나오면 새로고침이 실제 처방이다
  // (목록이 낡아 옛 종류로 요청한 경우가 대부분이다).
  TEMPLATE_KIND_MISMATCH: '계약서 서식 종류가 맞지 않아요. 화면을 새로고침하고 다시 시도해 주세요.',
  // 조항형 발송이 문서를 만들다 멈춘 경우들 — 셋 다 "서식을 고치면 풀린다"로
  // 수렴하지만 무엇을 고쳐야 하는지가 다르다.
  COMPOSE_DOCUMENT_INVALID:
    '계약서 내용을 불러오지 못했어요. 계약서 템플릿에서 내용을 확인하고 다시 저장해 주세요.',
  COMPOSE_RENDER_FAILED:
    '계약서를 만들지 못했어요. 잠시 후 다시 시도하고, 계속되면 고객센터로 알려 주세요.',
  // 폰트에 없는 문자(주로 한자)가 섞이면 PDF 에 빈칸으로 찍힌다 — 발송 전에 막는다.
  COMPOSE_UNSUPPORTED_CHARACTER:
    '계약서에 넣을 수 없는 문자가 있어요. 한자가 섞여 있다면 한글로 바꿔 주세요.',
  ...
  // 본인인증 도입 전에 만들어진 템플릿 — 역할 정책이 기본(이메일)이라 그대로
  // 보내면 강제가 조용히 꺼진 계약이 나간다. 스크립트 마이그레이션 대신 이
  // 문구가 사용자를 재저장으로 보낸다(재생성 경로가 easy_cert 를 심는다).
  TEMPLATE_AUTH_NOT_ENFORCED:
    '이 계약서 템플릿은 본인인증 설정 전에 만들어졌어요. 계약서 템플릿에서 열어 다시 저장하면 보낼 수 있어요.',
