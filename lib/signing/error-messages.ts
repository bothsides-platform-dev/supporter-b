...
export const SIGNING_ERROR_MESSAGES: Record<string, string> = {
  ...
  // 발송 경로 전용이다 — 이미 저장된 서식을 발송하려다 문서가 못 쓰게 된 경우.
  // 편집기의 저장 거절에는 쓰지 않는다(사용자는 아무것도 불러온 적이 없고 이미
  // 템플릿 화면 안이라 "템플릿에서 확인하고 다시 저장" 이 제자리를 맴돈다).
  // 저장 거절은 아래 세 코드가 원인별로 나눠 가진다.
  COMPOSE_DOCUMENT_INVALID:
    '계약서 내용을 불러오지 못했어요. 계약서 템플릿에서 내용을 확인하고 다시 저장해 주세요.',
  // ── 조항형 저장 거절 (원인별) ──
  // 셋 다 "지금 화면에서 고칠 수 있는 것"을 말한다.
  COMPOSE_UNKNOWN_TOKEN:
    '자동 입력 목록에 없는 {{토큰}}이 있어요. 오타를 고치거나 목록에 있는 토큰으로 바꿔 주세요.',
  COMPOSE_TOKEN_IN_HEADING:
    '조 제목에는 {{토큰}}을 넣을 수 없어요. 제목은 자동 입력되지 않아 그대로 인쇄돼요 — 토큰은 본문으로 옮겨 주세요.',
  COMPOSE_DOCUMENT_TOO_LARGE: '계약서가 너무 길어요. 조항 수나 본문을 줄여 주세요.',
