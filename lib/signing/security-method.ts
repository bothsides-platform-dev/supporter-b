 ...
 *
 * 전화번호 **포맷** 유틸(`isCompletePhone`·`normalizePhone`)은 `01[0-9]` 를 넓게
 * 허용한다 — 구 번호로 가입한 계정이 이미 있고 그 표시·정규화 경로가 계속 이들을
 * 타기 때문이다. 좁히는 것은 여기 하나뿐이고, 넓은 판정을 그대로 쓰면 차단이 아니라
 * 발송 400 이 되어 딜이 원인 없이 멈춘다.
 *
 * **이 규칙의 단일 출처는 이 파일이다.** 번호를 받는 화면(가입 프로필·설정 > 프로필)과
 * 저장하는 액션(가입 3종·`updateMyPhoneAction`)이 전부 `resolveSecurityMethod` 를
 * 직접 부른다. 예전에는 컴포넌트가 자체 정규식(`/^010\d{7,8}$/`)을 들고 있어 10자리를
 * 통과시켰고, 그 번호가 SMS 와 OTP 왕복을 다 거친 뒤에야 거절됐다 — 파생 상수를
 * 두지 않는 것이 그 드리프트를 표현 불가능하게 만든다.
 */
