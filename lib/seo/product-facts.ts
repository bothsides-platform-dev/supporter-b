...
export const PRODUCT_NAME = '서포트 B';

const BUYER_FACTS: AudienceFacts = {
  summary:
    '구매사(가맹점)가 여러 PG사(결제대행사)의 카드 수수료·정산 조건 견적을 한 번에 비교해 최적 조건으로 계약하는 봉인 입찰 견적 플랫폼입니다.',
  intro:
    '서포트 B에서 구매사가 카드 수수료·정산 조건을 담은 견적 요청서(RFP)를 발행하면, 복수의 PG사가 봉인 입찰(sealed bid)로 견적을 제출하고 구매사가 조건을 비교해 최적 파트너를 선정합니다. 단일 PG 견적만 받고 협의 없이 계약하던 기존 방식의 불투명한 수수료·제한된 협상 문제를 해결합니다.',
  facts: [
  ...
  links: [
    { title: '서포트 B 홈', path: '/', desc: '구매사용 서비스 소개' },
...
const PG_FACTS: AudienceFacts = {
  ...
  intro:
    'PG 영업에서 가장 어려운 건 리드 수가 아니라 확실한 니즈입니다. 서포트 B는 결제 도입·변경을 실제로 검토하는 고객사를 선별해, 업종·거래 규모·현재 조건·희망 조건이 정리된 RFP 형태로 파트너 PG사에게 전달합니다. 콜드콜이나 광고 리드보다 선명한, 검증된 영업 기회를 받습니다.',
  facts: [
    ...
    '계약 주체: 최종 계약은 고객사와 PG사가 직접 진행하고, 서포트 B는 비교·연결을 지원합니다.',
  ],
  process: [
  ...
  links: [
    { title: '서포트 B 파트너 홈', path: '/', desc: 'PG 파트너용 서비스 소개' },
