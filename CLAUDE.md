...
- **선정 후 전자서명 (SnowSign Templates)**: 선정(award)에서 흐름이 끝나지 않고 서포트비 안에서 전자서명까지 잇는다. **PG가 자사 계약서를 스노우싸인(SnowSign) 템플릿으로 등록·링크**(`/signing-templates`, 서명칸·좌표는 SnowSign 소유 — 앱은 PDF 에디터·좌표 저장 없음)해 두고, **어떤 계약서를 보낼지는 견적마다 고른다.** **자동 발송은 없다** — 구매사가 선정하는 순간 `awardRfpAction`이 `ContractSigningService.onAward`를 호출하지만 이는 **항상** `awaiting_pg_template` 행을 만들 뿐이고(견적에 계약서를 골라뒀든 아니든 예외 없음), 실제 발송은 PG가 딜룸 계약 탭의 픽커에서 계약서를 확인하고 `sendContract`를 실행할 때 한 번 일어난다. 견적 제출 시 고른 계약서(`bids.signing_template_id`, 선택 사항)는 그 픽커의 기본 선택으로만 쓰인다. **워크스페이스 '기본 템플릿'(`is_default`) 개념은 없다** — 예전엔 그 플래그로 award 시 자동 발송했으나, 유일성 제약이 없어 템플릿이 둘이면 어느 것이 나갈지 비결정적이었다(제거됨). 양측은 이메일 링크의 SnowSign 페이지에서 서명하고, 앱은 **SnowSign 웹훅(저지연 트리거) + 폴링(백스톱)**으로 상태를 반영한다: `POST /api/signing/webhook`이 HMAC-SHA256 서명(`X-Webhook-Signature`)을 검증한 뒤 payload의 `contract_id`만 뽑아 `reconcileByProviderRef`→`reconcileStatus`(getContract 재조회)로 위임하고 — 웹훅은 payload를 신뢰하지 않는 순수 트리거이며 상태 매핑은 폴링과 동일한 단일 경로, 멱등 `ensureFinalized`로 중복 무해 — 웹훅 유실(SnowSign auto-retry 없음) 대비로 딜룸 lazy + `poll-signing-status` cron 폴링이 백스톱을 이룬다. 웹훅 시크릿은 `SNOWSIGN_WEBHOOK_SECRET`(미설정 시 401 fail-closed로 웹훅 무시, 폴링만 동작). 딜룸의 전용 `계약` 탭(`SigningTab` — `signing` 이 있으면 탭 맨 앞·기본 활성, 레일 도트 + 다른 탭의 `SigningSummaryStrip` 한 줄이 상태를 알린다)이 8개 상태를 상태 헤더 · 세로 서명 타임라인 · (상태별) 계약서 픽커 또는 완료 문서 · 액션 바 고정 순서로 보여주고(픽커와 문서는 상호배타라 렌더 구역은 항상 셋) 리마인더·취소·재발송·완료본 온디맨드 다운로드(1h URL 302 프록시, 로컬 보관 없음)를 노출한다. 상태 파생은 `components/deal-room/signing/signing-view-model.ts` 순수 함수가 단일 출처이며, 카드 전체(제목·칩·액션)가 역할별로 갈리는 것은 `awaiting_pg_template` 한 상태뿐이다 — PG 쪽은 등록된 템플릿이 있으면 계약서 픽커 + '이 계약서로 보내기', 없으면 템플릿 등록 CTA 로 다시 갈린다. **픽커는 PG 경로에만 실린다**: `buildSigningCardView(signing, side, ctx?)` 의 3번째 인자가 PG 전용이고, 구매사 로더(`BuyerRfpDetailData`)에는 대응 필드 자체가 없다. 칩 라벨(`계약서 보내기 전`)은 ctx 에 의존하지 않는다 — `buildSigningSummary` 가 ctx 없이 카드뷰를 다시 만들기 때문. 단, 선정 마일스톤 라벨(`awardedNode` — 구매사 '견적을 선정했어요' / PG '이 견적이 선정됐어요')은 `awaiting_pg_template` 외에 `send_failed`·발송 전 취소된 `canceled`에서도 역할별로 문구가 갈린다. (`send_failed` 는 **레거시 렌더 전용** — 리스 CAS 도입 후 이 상태를 쓰는 코드가 없다. 발송 실패는 awaiting 유지 + 클레임 해제로 처리된다.) **org 스코핑**: 단일 `SNOWSIGN_API_KEY`=1 org 라 `GET /v1/templates` 원본을 PG에 노출하지 않고 `pg_signing_templates` 링크분만 스코프. ACL: 계약 조회·조작 = 낙찰 PG ws + buyer ws(로더는 buyer 항상 / PG `awardedToMe`만). 완료 칩은 '서명 완료', buyer 대기 라벨은 '**PG사가 계약서 준비 중**'. **봉인 경계**: 견적이 고른 계약서는 구매사에게 선정 전후 어느 시점에도 노출되지 않는다 — `signingTemplateId` 를 `Bid` 도메인 타입과 `BID_COLUMNS`/`rowToBid` projection 에 넣지 않고(구매사 비교표가 `Bid[]` 를 그대로 읽는다) `BidRepo.findSigningTemplateId` 좁은 경로로만 읽는 것이 구조적 보장이다. 얕은 `SnowSignClient` seam(교체 용이성은 YAGNI로 의도적 완화) + `ContractSigningService` 파사드. 레거시 award 기록 테이블 `contracts`와는 별개(`signing_contracts`). **동시성·경계 하드닝(리뷰 반영)**: 웹훅은 `after()`로 응답 후 재조회(5초 예산 비블로킹) + `provider_ref` 인덱스; 종결 전이(declined/expired)·resend 취소는 원자 CAS(`transitionIfActive`)로 동시 폴링·웹훅 중복 알림/완료본 클로버 방지; `performSend`는 발송 후 로컬 영속 실패 시 SnowSign 계약을 보상 취소(고아 방지); 딜룸 발송은 `claimForSend` 리스 CAS(`claimed_for_send_at`, 5분 — `SEND_LEASE_MS`)로 직렬화해 PG 담당자 둘이 동시에 눌러도 SnowSign 계약이 한 건만 생긴다 — 상태는 안 바꾸므로 실패 시 계약은 awaiting 에 남고 클레임만 풀려 바로 재시도할 수 있다; 재발송(`resend`)은 직전에 실제로 쓴 계약서를 재사용해 즉시 보내고(양측 모두 가능), 그 템플릿이 삭제됐거나 타 워크스페이스 소유면 에러 대신 새 라운드 awaiting 으로 저하한다; 템플릿 링크·조회는 크로스-테넌트 소유 가드(`findBySnowsignTemplateId` — 타 PG 링크분 거부/차단, 미링크 신규분 첫 조회는 Phase 11 소유검증 대기); `getForActor` ACL-먼저(award 존재 오라클 제거); 참여자 이메일 대소문자 무시 매칭; poll cron 이 방치 awaiting 을 7일 스로틀로 재넛지; cron 인증 상수시간·헤더 전용.
...
│  └─ contract-signing.ts # ContractSigningService: 선정 후 전자서명(SnowSign Templates) — onAward(항상 대기)/sendContract(유일한 발송 경로)/reconcile/cancel/remind/resend/renameTemplate/deleteTemplate/getForActor/getDownloadUrl. 얕은 SnowSignClient(lib/server/signing/) 파사드
