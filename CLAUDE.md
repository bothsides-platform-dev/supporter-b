...
- **전자계약**: 선정(awarded) 후 선정된 PG가 자사 계약서 PDF 템플릿으로 전자계약서를 발송하면, buyer↔PG 지정 서명자가 플랫폼에서 서명한다. 서버가 템플릿 본문 + **[별지1] 계약 개요**(선정 견적 조건·당사자, "본문 우선" 명문) + **[별지2] 전자서명·감사추적 확인서**(서명 이미지·동의 문구 v1·IP·KST 일시·이벤트 타임라인·base PDF SHA-256)를 자동 합성하고, 전 페이지 하단에 문서번호(`CT-YYMM-NNNN`)와 페이지 각인(간인 대체)을 남긴다. 상태는 `발송됨(서명 대기)`→`서명 완료`|`반려`(buyer)|`회수`(PG)|`만료`(기본 14일 lazy 만료, 1~90 설정)이며, RFP당 활성 문서 1건(완료 후 재발송은 변경계약으로 허용). 위변조 여부는 SHA-256 재계산으로 검증 배지 표시. **런치 게이트**: 마스터 계정에만 프론트 노출(`lib/features/e-contract.ts`, env `E_CONTRACT_ALL=1` 시 전체 공개 — `OPEN_BOARD_ENABLED` 킬스위치와 동일 패턴의 사용자 스코프 버전). 신규 5테이블(`contract_templates`/`contract_docs`/`contract_doc_signers`/`contract_doc_events`/`contract_doc_counters`)은 기존 `contracts`(선정 기록) 테이블과 별개다. (v0.4.0.0)
...
│  ├─ contracts/                 # buyer+pg 공용 — 전자계약 (목록·뷰어+서명, PG 생성 /contracts/new)
│  ├─ contract-templates/        # pg — 계약서 PDF 템플릿 관리
