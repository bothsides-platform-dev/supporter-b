...
- If you edit DESIGN.md tokens → also bump `styles/tokens.css`. The canvas token (`--md-sys-color-background`) additionally drives browser/PWA chrome (`app/layout.tsx` `viewport.themeColor`, `app/manifest.ts`) — `app/__tests__/chrome-colors.test.ts` reads `tokens.css` directly and fails if they drift. See DESIGN.md §2.
