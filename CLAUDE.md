...
│  ├─ home/                   # buyer+pg 홈 대시보드. buyer 홈은 워크스페이스 RFP 0건일 때만 `FirstRfpCoachmark`(components/onboarding/FirstRfpCoachmark.tsx, action-kind 단일 스텝)를 노출 — CTA(`home-create-rfp`) 실클릭 시 `users.onboarding.buyerFirstRfp.completedAt`, 말풍선 건너뛰기 클릭 시 `dismissedAt` 스탬프(lib/onboarding/visibility.ts). 코치마크 인프라(`CoachmarkTour`/`CoachmarkOverlay`)는 `components/onboarding/coachmarks/`에 유지 — PG 워크스페이스 온보딩 코치마크는 아직 없음(추후 과제)
