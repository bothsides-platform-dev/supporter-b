...
- **Bidding is sealed 1:N; discovery is open (opt-out)**: participation (who can bid) is buyer-controlled via the workspace-ID allowlist (`rfp_allowed_pg`), and **bids stay sealed — PGs never see each other or a competitor count (`Bid.competitorCount` does not exist by design).** Two axes, kept separate:
  ...
  - **Per-field opt-out for invited PGs — `현재 카드 수수료`.** Even an invited PG who sees the full brief can be denied one field: the buyer's **current card fee** (`current_fee_visible_to_pg`, default true = shown). When off, the value is stripped server-side in `loadPgRfpDetail` (the PG never reads it from the RSC payload/network — `RfpBriefPanel`'s render gate is only the visual fallback). The buyer's own comparison baseline always keeps the fee. Toggle lives in the RFP create wizard (step 2, under 현재 카드 수수료). **Implementation note (v0.2.26.0+):** the strip mechanism generalized from the single `current_fee_visible_to_pg` boolean to a `hidden_from_pg` field-path allowlist on `rfps` (paired with a versioned `current_terms` JSONB brief in `lib/types/rfp-terms.ts`). `loadPgRfpDetail` strips any path in `hidden_from_pg` that has a handler in `PG_STRIP` (fail-closed; the legacy boolean is still honored as a fallback during the Phase B–F migration). User-facing behavior is unchanged — this is groundwork for hiding more fields without a DDL change. **The default `현재 조건` brief fields still live in their own `current_*` columns until Phase F; `current_terms` is read-authoritative with a column fallback.**
