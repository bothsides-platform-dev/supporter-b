...
export function useThemeToggle() {
  ...
  /**
   * 테마를 뒤집으며 원형 리빌을 시작한다.
   *
   * 리빌 원점은 `from` 안의 **첫 번째 `<svg>`** 중심이고, svg 가 없으면 `from`
   * 자체로 떨어진다. 아이콘이 여럿인 버튼(배지·chevron·스피너를 함께 그리는
   * 경우)이라면 문서 순서상 첫 번째가 잡히므로, 그런 소비처가 생기면 원점을
   * 명시로 받도록 시그니처를 넓혀야 한다.
   */
