...
describe('SignupEmailGuide', () => {
  it('입력 전에는 중립 힌트를 보여준다', () => {
    ...
    expect(guide).not.toHaveTextContent('개인 이메일이에요');
  });
  ...
  it('무료 도메인 감지 시 amber 경고로 전환한다', () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('개인 이메일이에요. 회사 이메일로 가입하면 팀원과 함께 쓰기 쉬워요.');
  });
  ...
  it('힌트→경고 전환에도 라이브 리전 엘리먼트가 유지된다 (스크린리더 안내 보장)', () => {
    ...
    expect(region).toHaveTextContent('개인 이메일이에요');
  });
