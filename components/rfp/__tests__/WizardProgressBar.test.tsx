...
describe('WizardProgressBar', () => {
  it('현재 단계 레이블과 Step X/3 텍스트를 표시한다', () => {
    render(<WizardProgressBar currentStep={2} completed={[true, false, false]} />);
    expect(screen.getByText('Step 2 / 3 — 견적 내용')).toBeInTheDocument();
  });

  it('총 3개의 dot을 렌더한다', () => {
    render(<WizardProgressBar currentStep={1} completed={[false, false, false]} />);
  ...
  it('완료된 step(현재 step 아님)의 dot은 done 표시(data-done)를 가진다 — 위치 무관', () => {
    ...
    render(<WizardProgressBar currentStep={1} completed={[false, false, true]} />);
  ...
  it('기본값: 3단계 dot + 구매사 라벨', () => {
    render(<WizardProgressBar currentStep={1} completed={[false, false, false]} onStepClick={vi.fn()} />);
    expect(screen.getAllByTestId('progress-dot')).toHaveLength(3);
  ...
  it('failedAt 있는 미완료 비활성 dot에 data-error="true" 속성이 적용된다', () => {
    ...
    render(<WizardProgressBar currentStep={1} completed={[true, false, false]} failedAt={[false, true, false]} />);
    const dots = screen.getAllByTestId('progress-dot');
  ...
  it('failedAt 없으면 미완료 비활성 dot도 data-error="false"이다', () => {
    ...
    render(<WizardProgressBar currentStep={1} completed={[true, false, false]} />);
    const dots = screen.getAllByTestId('progress-dot');
  ...
  it('steps prop: 단계 수·라벨이 바뀐다', () => {
    ...
    // 기본 3단계 라벨은 "PG 선택·최종 확인"이므로 "검토·발송"이 보이면 steps prop이 적용된 것.
    render(
      <WizardProgressBar
        currentStep={4}
