...
function makeConv(overrides?: Partial<ConversationListItem>): InboxListItem {
  const c: ConversationListItem = {
    ...
    counterparty: { workspaceId: 'ws-pg', name: 'NICE페이', type: 'pg', hasLogo: false, logoUpdatedAt: null },
    rfpId: null,
    rfpCode: null,
    rfpTitle: null,
    rfpStatus: null,
  ...
  it('shows at most 4 rows (HOME_RECENT_MESSAGES cap)', () => {
    const convs = Array.from({ length: 5 }, (_, i) =>
      makeConv({
        ...
        counterparty: { workspaceId: `ws-${i}`, name: `회사 ${i}`, type: 'pg', hasLogo: false, logoUpdatedAt: null },
      }),
    );
