 ...
 * — `app/(app)/settings/profile/page.tsx` 가 DB의 승인된 관리자 멤버십
 * (또는 운영계정)을 확인해 prop 으로 내려주는 배선 — 은 어느 유닛도 통과하지 않는다.
 * prop 을 거꾸로 넘기거나 상수 true 로 두면 유닛은 전부 green 인 채로 일반
 * 멤버에게 수정 버튼이 다시 보인다.
 *
 * 권한 판정은 JWT 역할이 아니라 현재 DB 멤버십을 보므로, 역할만 바꾸고
 * 새로고침하면 재로그인 없이 반영된다.
 */
...
test.describe('설정 — 워크스페이스 등록정보 admin 게이트', () => {
  ...
  test('일반 멤버에게는 수정 버튼이 없고, admin 에게는 있다', async ({ page }) => {
    ...
    await expect(page.getByText('워크스페이스 정보', { exact: true })).toBeVisible();

    // **사업자번호 행에 스코프해야 한다.** 다른 항목이 '수정' 버튼을 추가해도
    // 이 폼의 게이트가 잘못 열리거나 닫힌 것을 정확히 잡아야 한다.
    ...
    await expect(wsSectionAdmin.getByRole('button', { name: '로고 변경' })).toBeVisible();
    await expect(wsSectionAdmin.getByRole('button', { name: '변경 요청' })).toBeVisible();
    ...
    // 이름 폼도 같은 게이트를 지난다.
    await expect(workspaceSection(page).getByRole('button', { name: '변경 요청' })).toHaveCount(0);

    // 로고 행도 같은 게이트를 지난다.
    const wsSection = workspaceSection(page);
    await expect(wsSection.getByRole('button', { name: '로고 변경' })).toHaveCount(0);
