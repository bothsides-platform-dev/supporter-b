# bidit · PG 비공개 RFP 플랫폼 — Design Spec (v0)
...
**bidit** 은 구매사가 (선택) 사업자번호 자동 enrichment 와 함께 RFP 를 작성해 본인이 허용한 PG 들에게 이메일로 발송하고, PG 는 가입 강제 워크스페이스 안에서 정형 제안을 제출하며, 구매사는 6개 정형 수치 + PDF 첨부로 비교한 뒤 선택하는 **비공개 1:N RFP 플랫폼**이다. 사업자번호·등급은 옵셔널 — 법인 미설립 사전 제안과 마감 압박 시 보완 입력 두 시나리오를 모두 지원한다.
...
| PG 영업담당 | `pg` 도메인 자동 통합 (`@toss.im` → 토스페이먼츠) | RFP 수신 → 제안 작성 → 제출 |
...
| 11 | PG 워크스페이스 정체성 = **이메일 도메인** (자동 라우팅). RFP 접근권은 **초대된 PG 워크스페이스의 모든 멤버**에게 부여(2026-05-10 정책 변경 — 클레임 사용자 단위 차단 폐기) |
...
| C3 | `/invite/rfp/:token` | RFP 초대 메일 링크 진입점. 토큰 검증 → 가입/로그인 → 도메인 자동 라우팅 → RFP 수신함 |
| C4 | `/logout` | |
...
2. [수락·제안 작성] → 토큰 검증 → `/login` (next 보존)
...
| **RFP 수정** | 발송 후 메모·첨부 수정 가능, PG 에 "RFP 변경됨" 알림. 마감일 연장 가능. PG 이메일 추가만 가능 (제거 불가). |
