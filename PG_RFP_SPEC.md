...
| PG 영업담당 | `pg` 워크스페이스 (가입 시 이름 입력 — 서포터 B 페이 등) | RFP 수신 → 제안 작성 → 제출 |
...
5. PG 워크스페이스 검색·선택 (Popover + 검색 입력): `서포터 B 페이`, `KG이니시스`, `NICE페이먼츠`
6. 마감 D+7 → 발송
...
### 시나리오 B — PG 제안 응답
1. 서포터 B 페이 영업담당 메일함: `노온 → 서포터 B 페이 RFP #abc123` 수신
2. [수락·제안 작성] → 토큰 검증 → 기 가입자는 `/login`(next 보존) 후 인박스로, 미가입자는 `/signup/pg` funnel로 분기
3. 가입 시 워크스페이스 이름 입력 (이미 서포터 B 페이 ws 존재 시 그 ws에 합류 신청 / 신규 ws 생성 선택)
...
3. (옵션) `[ 보드 ]` 토글 → 칸반 뷰. 카드 드래그 또는 ⋯ 메뉴로 서포터 B 페이를 `협상중` → `결정`으로 이동. 카드 클릭으로 상세 모달 열어 협상 메모·스크린샷 누적.
4. 서포터 B 페이 Bid 선택 → [수주 처리]
5. 서포터 B 페이에 "수주 확정" 메일, 미선택 2곳에 "이번엔 다른 PG 선택" 메일
