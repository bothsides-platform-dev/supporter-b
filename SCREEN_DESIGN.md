...
- **추천 준비 화면 (2026-09-26)**: 실제 구매사 최종 확인 단계에서 앱 셸·단계 안내를 유지하고 작성 영역에 최소 10초 연출을 표시한다. 제목·설명에 선택 업종을 밝히고 `아직 상담 요청을 보내지 않았어요. 추천 결과를 보고 PG사 한 곳을 고를 수 있어요.`로 다음 결정 시점을 안내한다. 제목 위 아이콘과 퍼센트는 없고, 세 단계는 실제 응답과 경과 시간을 함께 확인해 대기·펄스 점·완료 체크로 전환한다. 사업자 정보 단계는 등록 여부 조회가 끝난 뒤, 상담 조건·추천 준비 단계는 추천 조회가 끝난 뒤에만 완료로 표시한다. 마지막 단계의 PG 브랜드 로고는 기존 가로 연출을 유지한다(로고 순서는 장식이며 실제 후보·PG의 심사 참여를 뜻하지 않는다). 빠른 응답도 10초 뒤에 결과를 보여주며 느린 응답은 도착할 때까지 기다린다. 오류는 즉시 재시도·이전 단계 복귀를 제공한다. 완료 후 실제 후보 선택·마감일·요청 요약을 함께 표시하며 PG 선택 전에는 상담 요청 버튼을 비활성화한다. 업종 변경·재시도는 상태와 시계를 초기화하고 이전 응답을 무시한다. 모션 감소 설정에서는 점과 로고를 정지한다. 랜딩·튜토리얼 샘플 흐름은 적용 대상이 아니다.
