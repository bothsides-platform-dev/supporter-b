...
- **견적 수정 요청 (2026-09-27)**: 구매사는 이미 제출한 PG사의 견적에 수정 내용과 대상별 응답 마감을 보낸다. 맞춤 상담에서는 해당 PG사를 기본 대상으로 고르고, 기존 1:N에서는 선택한 PG사에만 적용한다. 공용 견적 요청 마감은 바꾸지 않는다. 만료된 미응답 요청은 같은 요청의 마감을 연장할 수 있으며 이전 메시지·마감은 활동 기록에 남긴다. PG사는 기존 조건·메모·등급별 요율을 바탕으로 새 회차를 작성하고 기존 PDF 유지·새 PDF 교체·제거를 선택한다. 기존 PDF를 유지하면 원본 첨부의 소유권은 그대로다. 새 견적을 보낼 때까지 기존 견적은 비교·선정 대상이며, 구매사가 수정 견적을 기다리지 않고 선정하면 확인창에서 이를 알린다. 재제출은 현재 수정 요청 ID·마감 판본·기준 견적 ID를 서버에서 확인해 낡은 탭이 다음 요청을 소비하지 못한다. 양측은 제출된 각 회차의 작성자·시각·조건·메모·첨부를 읽기 전용으로 본다. PG에게는 자기 회차만 제공하고 구매사 비교·선정은 PG별 최신 회차를 기본으로 삼는다.
- **맞춤 PG 상담 (확정 결정 2026-09-19)**: 사업자 정보 확인 → 견적 내용·업종 입력 → 작성 영역의 최소 10초 추천 준비 화면(사업자 정보 등록 여부 → 업종별 상담 조건 확인 → 추천 PG사 준비) → 관리자 기준에 따른 PG 추천 → **한 곳에 상담 요청** → PG 검토·견적 또는 거절 → 구매사의 견적 확인·**최종 선정** → 기존 계약서 준비. 첫 PG 선택은 선정이 아니며 계약을 만들지 않는다. 기존에 발송한 1:N 견적과 랜딩·튜토리얼 샘플은 종전 흐름을 유지한다.
- **한국 영업일 견적 마감 (확정 결정 2026-09-26)**: 실제 구매사 상담의 기본 마감은 요청 다음 날부터 5영업일, 빠른 선택은 3·5·10영업일이다. 직접 고른 날짜도 최소 3영업일 이후·요청일로부터 30달력일 이내의 한국 영업일이어야 하며 해당일 18:00 KST에 마감한다. 날짜 버튼은 한국어 팝업 달력을 열고, 유효하지 않은 날과 해당 화면의 연장 기준보다 이른 날은 선택할 수 없게 표시한다. 선택 가능한 첫 날짜와 휴일 제외를 안내하며, 자정·달력 갱신으로 선택이 무효가 되면 자동 변경 없이 재선택을 요청한다. 토·일, 공식 공휴일(설·추석·대체·임시 포함), 5월 1일과 운영 예외일을 건너뛴다. 미적재 달력은 날짜를 추정하지 않고 선택·발송을 막는다. 달력 데이터는 구매사 작성 화면과 딜룸 마감 변경 행동에 공통으로 쓰며 서버가 쓰기 시점에 재검증한다. 새 휴일 발표로 이미 저장된 마감이 휴일이 돼도 자동 연장하지 않고 구매사에 변경 필요를 안내한다. 받은 견적 요청·상담 상태·재요청 응답은 해당 PG에 실제로 적용되는 마감을 표시하고, 지나면 제출을 막는다. 기능 플래그는 최초 달력 적재 뒤에만 켜며 배포·운영 규칙은 `docs/BUSINESS_DEADLINES_ROLLOUT.md`를 따른다. 기존 발송된 1:N 견적과 랜딩·튜토리얼 예시는 기존 마감 규칙을 유지한다.
- **추천 준비 화면 (2026-09-26)**: 실제 구매사 최종 확인 단계에서 앱 셸·단계 안내를 유지하고 작성 영역에 최소 10초 연출을 표시한다. 제목·설명에 선택 업종을 밝히고 `아직 상담 요청을 보내지 않았어요. 추천 결과를 보고 PG사 한 곳을 고를 수 있어요.`로 다음 결정 시점을 안내한다. 제목 위 아이콘과 퍼센트는 없고, 세 단계는 실제 응답과 경과 시간을 함께 확인해 대기·펄스 점·완료 체크로 전환한다. 사업자 정보 단계는 등록 여부 조회가 끝난 뒤, 상담 조건·추천 준비 단계는 추천 조회가 끝난 뒤에만 완료로 표시한다. 마지막 단계의 PG 브랜드 로고는 기존 가로 연출을 유지하고, `PG 로고는 예시예요. 잠시 후 실제 추천 결과를 보여드릴게요.`로 장식용 로고와 실제 추천 결과를 구분한다. 빠른 응답도 10초 뒤에 결과를 보여주며 느린 응답은 도착할 때까지 기다린다. 오류는 즉시 재시도·이전 단계 복귀를 제공한다. 완료 후 실제 후보 선택·마감일·요청 요약을 함께 표시하며 PG 선택 전에는 상담 요청 버튼을 비활성화한다. 업종 변경·재시도는 상태와 시계를 초기화하고 이전 응답을 무시한다. 모션 감소 설정에서는 점과 로고를 정지한다. 랜딩·튜토리얼 샘플 흐름은 적용 대상이 아니다.
- **표준 업종 선택·직접 입력 (2026-09-24)**: 실제 구매사 업종 질문은 카테고리 진입 후 쉬운 업종명·예시의 한 열 라디오 목록과 `찾는 업종이 없어요 · 직접 입력`을 제공한다. 구매사 목록에는 MCC를 표시하지 않으며 코드 검색은 지원한다. 빈 목록에서도 직접 입력할 수 있고 검색 결과가 없으면 검색 초기화·직접 입력을 안내한다. 직접 입력 선택 시 검색어를 가져오며 방식 전환·이전 질문 복귀에도 입력을 보존하되 현재 방식만 제출한다. 이름은 앞뒤 공백 제거 후 1~100자이며, 서버는 공백 정리·대소문자 정규화 후 기존 이름과 일치하면 해당 정책(Black 포함)을 적용한다. 미일치 이름은 상담에만 저장하고 기본 PG를 Gray로 추천한다. 기본 PG 미설정 시 작성 내용은 유지하고 문의 경로를 제공한다. 최종 확인·PG 요청 조건에는 업종명을 표시한다. 표준 29개는 관리자 카탈로그의 일회성 등록이며 코드·이름 중복과 기존 정책을 보존한다. 이후 삭제는 자동 복원하지 않는다.
- **업종 탐색 (2026-09-26)**: 구매사와 별도 관리자 `/pg-recommendations`는 카테고리 → 쉬운 업종명·예시로 탐색하며 상품·서비스 검색은 전체 범위에 적용한다. 구매사는 등록 업종 단일 선택과 직접 입력을, 관리자는 미등록 표준 업종 복수 등록과 기존 편집 폼을 유지한다. 카테고리만 선택하면 작성 완료가 아니다. 표시 메타데이터·동기화 절차는 `docs/INDUSTRY_NAVIGATION.md`를 따른다.
- **추천 운영 기준**: `pg_matching_policies`가 업종별 White(일반)/Gray(추가 검토)/Black(접수 불가)와 PG 순서·추천 사유·영세 예상 수수료·조건을 소유한다. 업종별 유효 후보가 없으면 관리자가 지정한 기본 PG를 Gray(추가 검토)로 제안하며, 구매사가 직접 선택해야 상담이 시작된다. 기본 PG가 미설정이거나 후보가 모두 소진되면 문의 경로를 제공한다. Black 업종은 기본 PG로 우회하지 않는다. 같은 PG를 여러 업종에 등록할 수 있다. 키움·헥토·2차 PG의 수용 정책을 이름으로 추정하지 않는다. 관리자 콘솔은 별도 레포 `/pg-recommendations`다.
- **거절·철회 후 다음 추천**: 진행 중인 상담은 한 곳뿐이며 과거 상담 PG는 재추천하지 않는다. 다음 요청 시 최신 정책과 활성 PG 여부를 다시 확인하고 새 마감일을 저장한다. 후보가 없거나 등록 업종이 삭제되면 운영팀 문의로 이어진다. 직접 입력 상담은 기본 PG로 재추천하되 이전 상담 PG는 제외한다. 거절 사유와 PG명·제안 조건 스냅샷은 구매사 상담 이력에 남는다. PG에게는 자기 검토 결과만 제공한다.
...
**구매사 업종 입력 (2026-09-24)**: 실제 견적 작성은 업종 질문을 항상 표시하며 §0의 표준 업종 선택·직접 입력 규칙을 따른다. 등록 업종 ID 또는 직접 입력 이름 중 현재 방식 하나를 제출한다. 이전·다음 이동과 새로고침에서도 초안의 선택 방식과 입력을 유지한다. 업종을 건너뛴 기존 초안은 마지막 질문에서 업종 질문으로 돌아온다. 빈 목록은 직접 입력란을 바로 표시하며 작성 진행을 막지 않는다. 추천 가능한 PG가 없는 경우에만 추천 단계에서 문의 안내를 제공한다.
...
### PG 선정 후 합의서 진입점 (2026-09-22)

- 사이드바는 기존 구조를 유지한다. 작성은 `/inbox/:rfpId?tab=contract`의 계약 탭에서 한다.
- PG 홈의 할 일 맨 앞에 `계약서를 보내야 해요`와 건수를 표시한다. 선정된 PG의 최신 계약 회차 중 발송 전인 건만 구매사명·견적 제목·다음 행동과 함께 노출하며, 발송 후에는 빠진다. 조회 실패는 빈 할 일로 바꾸지 않고 오류 경계로 전달한다.
- 받은 견적 요청의 선정 행에는 작성 전 `합의서 작성하기`, 저장 후 `이어서 작성하기`, 발송 결과가 불명확하면 `발송 결과 확인하기`, 진행 중에는 `서명 현황 보기`, 완료 후에는 `완료 문서 보기`를 표시한다. 기존 계약·종결 상태는 `계약 상태 확인하기`로 연결한다. 모든 행동은 계약 탭 이동이며 자동 발송·재발송하지 않는다.
- PG 딜룸은 `요청 조건`을 기본 탭으로 유지하며 선정 안내와 계약 탭 이동 버튼을 상단에 제공한다. 계약 탭은 회사 정보만 편집할 수 있음을 안내하고, 저장한 초안을 이어서 연다.
- 저장·발송 성공 시 홈·목록·상세 캐시를 무효화한다. 목록은 상태·초안 판본·발송 준비 여부만 일괄 조회하며 회사 정보·문서 본문·공급자 ID를 내려주지 않는다.
...
**설정 화면 표시 규칙(B6·B7·P5·P6·S2):** 프로필·멤버·활동 기록·알림 설정은 `SettingsPage` 공통 틀을 사용한다. 받은 견적 요청 목록과 같은 `PageHeader`(48px 제목 행·하단 구분선)를 패널 상단 전체 너비로 배치하고, 본문은 중앙 정렬·최대 너비 제한 없이 좌우 24px 여백으로 제목과 왼쪽 기준선을 맞춘다. 페이지 행동은 헤더 오른쪽에, 설명·폼·목록은 본문에 배치하며 로딩·권한 안내·조회 실패도 같은 틀을 사용한다. 프로필은 라벨·값·수정 행동을 왼쪽 흐름으로 배치하고 사용자 사진과 워크스페이스 로고를 구분한다. 휴대폰 미등록은 인증 필요 칩과 기본 행동으로 알리되 오류로 취급하지 않으며, 워크스페이스 이름의 운영자 승인 안내는 편집 전에도 보인다. 멤버는 역할을 항상 표시하고 관리 메뉴는 분리하며, 최근 접속·서버에 저장된 초대 날짜를 명시하고 좁은 화면에서는 내용을 줄바꿈한다(새로 초대한 행은 재진입 전까지 `초대 완료`로 표시). 초대 요청 중에는 폼을 닫지 못하게 해 실패 안내와 입력을 보존한다. 활동 기록은 긴 내용을 자르지 않고, 저장된 메타데이터가 있는 사건에 한해 안전한 요약을 더한다(개인정보·원시 메타데이터 비노출). 더 보기 실패 시 기존 목록을 유지하고 일시 오류에는 재시도, 워크스페이스 전환에는 새로고침을 제공한다. 인증 확인 실패에는 재시도와 다시 로그인 경로를 함께 제공하고, 권한 상실은 안내만 한다. 기간·활동 종류 필터는 이번 화면 범위에 포함하지 않는다.
...
| `/inbox/:rfpId` | `PgReviewPanel`: 검토 시작·사유 입력·거절. 견적 마감 전 요청에서만 검토를 시작할 수 있다. 마감 뒤에는 시작 버튼을 숨기고 접수 종료 안내를 보여주며, 이미 검토 중인 상담의 거절은 허용한다. `검토 시작하기` 저장 성공 시 같은 딜룸의 `견적 작성` 탭으로 전환한다. 저장 중·실패 시 요청 조건을 유지하며 거절은 탭을 전환하지 않는다. 종결 상담의 견적 제출 차단. 자기 이력만 표시 |
