...
| P8 | `/quote-templates` | PG 워크스페이스 공유 견적 템플릿(요율표) 관리 — 정산조건+가입비+결제수단별 수수료율 프리셋 CRUD. 견적 작성(P3)에서 불러와 한 번에 채움 (최대 20개). 구간 수수료(카드·네이버페이·카카오페이·토스페이·애플페이·삼성페이) 직접 편집 지원. nav top 레벨 (G→Q). **셸은 P9 와 공유하는 리스트 페이지 문법**(`PageHeader` 스트립 = 제목 + 개수 칩 + 부제 + 액션, 내부 스크롤 본문) — 자세히는 P9 행. 목록이 비면 헤더 액션을 감추고 `EmptyState` 가 CTA(`새 템플릿 만들기`)를 소유한다. 저장 상한은 목록 하단 `Note`. 편집은 `Sheet`(base-ui Dialog — 스크림·Esc·포커스 트랩·닫기 버튼), 푸터는 취소 → 저장 순. 복제·삭제·저장은 성공·실패 모두 토스트(문구 SSOT `lib/quote/error-messages.ts`). | `QuoteTemplateList`, `QuoteTemplateDrawer`, `PageHeader`, `EmptyState`, `Note`, `Sheet` |
| P9 | `/signing-templates` | PG 전자서명 템플릿 설정(1회/PG) — 자사 계약서를 스노우싸인(SnowSign) Templates 로 등록·링크해 두면, 구매사 선정 시 그 템플릿으로 전자서명이 자동 발송된다. **셸은 P8(견적 템플릿)과 같은 리스트 페이지 문법이다 — 사이드바에서 나란한 두 PG 설정 페이지라 의도적으로 한 벌을 쓴다**: 공유 `PageHeader` 스트립(제목 + 개수 칩 + 부제 + 우측 액션) + `flex-1 overflow-auto px-6 py-4` 본문. 부제가 현재 단계를 안내하므로 별도 스텝 표시는 없다. **폭 규칙: 리스트는 full-bleed, 폼(embed·mapping)만 `max-w-[720px]`.** 빈 상태·목록도 공유 프리미티브(`EmptyState`·`divide-y` 리스트·`Note`)로 P8 과 동일. 상태 머신 3단계: **list**(링크된 템플릿 목록·`기본` 배지·org 스코프 안내, 빈 상태 제목 "아직 등록한 서명 템플릿이 없어요" + CTA "새 템플릿 만들기" — 목록이 있을 때만 헤더에 "새 템플릿") → **embed**(스노우싸인 `template_draft` 임베드 iframe — PDF 업로드·서명칸 배치·역할 정의는 스노우싸인 안에서, 앱은 좌표 미저장. 완료는 postMessage 자동 + "등록을 마쳤어요" 수동 폴백[template_id 입력]) → **mapping**(getTemplateDetail 로 불러온 템플릿 역할명 ↔ 구매사/PG 서명자, 변수명 ↔ 낙찰 견적 소스[정산주기·정산한도·가입비·보증보험 등] 연결 + 기본 지정 → linkSigningTemplate 저장, 저장 즉시 이 PG 낙찰 awaiting 계약 자동 발송). **org 스코핑**: 단일 API 키=1 org 라 `GET /v1/templates` 원본 비노출, `pg_signing_templates` 링크분만 표시(타 PG 템플릿 차단). nav top 레벨 (G→E). 세 액션(임베드 세션 발급·detail 조회·저장)은 `run()` 헬퍼가 try/catch + finally 로 감싼다 — throw 를 삼키지 않으면 `busy` 가 true 로 남아 화면의 모든 버튼이 새로고침 전까지 영구 비활성이 된다. | `SigningTemplateManager`, `Select`, `PageHeader`, `EmptyState`, `Note`, `Checkbox` |
