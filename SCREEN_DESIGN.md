...
별도 `admin-supporter-b` 콘솔의 `/pg-recommendations`는 업종 이름·표시 순서·PG 연결을 편집한다. PG 한 곳은 업종 하나에만 연결되며, 변경은 구매사 작성 화면의 추천에 반영된다.
