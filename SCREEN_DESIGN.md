...
├─ /signing-templates             (pg only — 계약서 템플릿 등록·이름 변경·삭제)
└─ /settings
   ├─ /settings/profile
   ├─ /settings/members
...
| B6 | `/settings/profile` | 구매사 사업자 프로필과 등급 갱신 상태. **사용자 섹션**: 프로필 사진 업로드·삭제(`UserAvatarForm`). **워크스페이스 섹션(로고·이름·사업자번호)은 admin 전용 편집**(v0.4.35.0) — `canEditWorkspace`(`isApprovedAdmin` + 마스터 계정은 `isMasterEmail` 로 면제, 멤버십 row 가 없어서) 한 술어를 세 폼에 그대로 넘긴다. 권한이 없으면 폼 대신 "워크스페이스 정보는 관리자가 바꿀 수 있어요" 안내만 뜬다 | `UserAvatarForm`, `WorkspaceLogoForm`, `WorkspaceNameForm`, `WorkspaceBizNoForm` |
...
| P5 | `/settings/profile` | PG 회사 정보 (워크스페이스 이름·연락처). **사용자 섹션**: 프로필 사진 업로드·삭제(`UserAvatarForm`). **워크스페이스 로고·이름은 admin 전용 편집**(v0.4.35.0, B6 와 동일한 `canEditWorkspace` 술어 — 사업자번호 폼은 buyer 전용이라 PG 화면에 없음) | `UserAvatarForm`, `WorkspaceLogoForm`, `WorkspaceNameForm` |
| P6 | `/settings/members` | 같은 워크스페이스 멤버 관리 (도메인 자동 합류 없음 — 초대만) | `MemberTable` |
| P8 | `/quote-templates` | PG 워크스페이스 공유 견적 템플릿(요율표) 관리 — 정산조건+가입비+결제수단별 수수료율 프리셋 CRUD. 견적 작성(P3)에서 불러와 한 번에 채움 (상한은 `MAX_QUOTE_TEMPLATES` — `lib/quote/limits.ts` 단일 출처. 서비스 강제·목록 하단 Note·`LIMIT_REACHED` 문구가 모두 여기서 파생하므로 숫자를 여기에 복제하지 않는다). 구간 수수료(카드·네이버페이·카카오페이·토스페이·애플페이·삼성페이) 직접 편집 지원. nav top 레벨 (G→Q). **셸은 P9 와 공유하는 리스트 페이지 문법**(`PageHeader` 스트립 = 제목 + 개수 칩 + 부제 + 액션, 내부 스크롤 본문) — 자세히는 P9 행. 목록이 비면 헤더 액션을 감추고 `EmptyState` 가 CTA(`새 템플릿 만들기`)를 소유한다. 저장 상한은 목록 하단 `Note`. 편집은 `Sheet`(base-ui Dialog — 스크림·Esc·포커스 트랩·닫기 버튼), 푸터는 취소 → 저장 순. **복제·삭제는 성공·실패 모두 토스트, 저장은 성공 토스트 + 실패는 드로어 본문 인라인**(저장 실패 시 드로어가 열린 채 남으므로 인라인이 보이는 표면이다). 문구 SSOT 는 `lib/quote/error-messages.ts`. | `QuoteTemplateList`, `QuoteTemplateDrawer`, `PageHeader`, `EmptyState`, `Note`, `Sheet`, `ConfirmDialog`, `Chip` |
| P9 | `/signing-templates` | PG 계약서 템플릿 — 자사 계약서를 스노우싸인(SnowSign) Templates 로 등록·링크해 두면, 견적을 보낼 때나 선정된 뒤 딜룸에서 골라 보낼 수 있다(자동 발송 없음). **셸은 P8(견적 템플릿)과 같은 리스트 페이지 문법이다 — 사이드바에서 나란한 두 PG 설정 페이지라 의도적으로 한 벌을 쓴다**: 공유 `PageHeader` 스트립(제목 + 개수 칩 + 부제 + 우측 액션) + `flex-1 overflow-auto px-6 py-4` 본문. 부제가 현재 단계를 안내하므로 별도 스텝 표시는 없다. **폭 규칙: 리스트와 embed 는 full-bleed, mapping 은 `max-w-[880px]`.** 예전 폼 공통 상한 `max-w-[720px]` 은 제거됐다. **폭을 푸는 이유는 embed 하나뿐이다** — 스노우싸인 위저드가 cross-origin iframe 이라 앱이 준 박스보다 넓게 리플로우할 수 없고, 따라서 폭 상한을 두면 그만큼이 그대로 위저드의 한계가 된다. 같은 이유로 높이도 하드코딩(`h-[460px]`)을 버리고 `flex-1` + `min-h-[460px]` 가드로 본문의 남은 세로 공간을 채운다(본문→`Panel`→카드 내부가 `flex flex-1 flex-col` 체인으로 이어져야 성립 — `Panel` 은 `className` 을 받는다. `min-h` 는 짧은 뷰포트 가드이고, 남은 높이가 그보다 작으면 본문의 `overflow-auto` 가 **카드째** 스크롤을 맡는다). **이 체인의 `Panel`·카드 내부에는 `min-h-0` 을 걸지 않는다 — v0.4.35.2 회귀의 원인이었다**: `min-h-0` 은 flex 자동 최소 크기를 0 으로 만들어 카드가 뷰포트 잔여 높이까지 줄어들게 하고, 그러면 `min-h-[460px]` 바닥에 걸린 iframe 이 `Note`·수동 폴백 컨트롤을 **카드 보더 밖으로** 밀어낸다(1240 폭 실측: vh=768 폴백 열림 42px, vh=660 닫힘 59px·열림 150px 유출 — 버튼이 빈 배경에 뜨고 보더가 그 사이를 가로지른다). 높이 채움은 `flex-1` 혼자로 충분하며 `min-h-0` 을 빼도 넉넉한 뷰포트의 iframe 높이는 동일하다(vh=1000 에서 724px, 폴백 열림 633px — 실측 동일). 본문 div 의 `min-h-0` 은 `overflow-auto` 가 이미 자동 최소 크기를 0 으로 만들어 무해하다(실측상 있으나 없으나 동일). 회귀 가드는 `components/signing-templates/__tests__/SigningTemplateManager.test.tsx` 의 「임베드 카드와 안쪽 열은 min-h-0 없이 flex-1 로만 높이를 채운다」 — jsdom 은 레이아웃을 계산하지 않으므로 원인 클래스를 구조로 못박는 방식이다(실브라우저 측정은 Phase 11 e2e 과제). **그 이유가 없는 것에는 폭을 풀지 않는다**: ① mapping 은 라벨→값 짝 짓기라 넓어질수록 읽기 어렵다(무제한이면 "PG사" 칩 470px·Select 658px 로 벌어진다) — 상한은 헤더 그리드·행 그리드·푸터를 함께 감싸는 열에 걸어 열 정렬을 지킨다(안쪽 그리드만 잡으면 `Panel` 보더가 남아 빈 오른쪽이 생긴다). ② 산문(`Note`·도움말 `<p>`)은 `max-w-[640px]` — `Note` 프리미티브 자체에 폭 상한이 없어서(`components/primitives/Note.tsx`) 상한을 안 주면 초광폭에서 한 줄로 화면을 가로지른다. 읽는 폭은 카드 폭과 별개다. ③ 한 줄 텍스트 입력 둘 — 매핑의 템플릿 이름(`max-w-[480px]`), embed 수동 폴백의 템플릿 ID(`max-w-[280px]`) — 둘 다 `flex flex-col` 안에서 stretch 되므로 상한이 없으면 화면 끝까지 늘어난다. 빈 상태·목록도 공유 프리미티브(`EmptyState`·`divide-y` 리스트·`Note`)로 P8 과 동일. 상태 머신 3단계: **list**(링크된 템플릿 목록 + 행 `[⋯]` 메뉴[이름 바꾸기·삭제]·org 스코프 안내, 빈 상태 제목 "아직 등록한 계약서 템플릿이 없어요" + CTA "새 템플릿 만들기" — 목록이 있을 때만 헤더에 "새 템플릿") → **embed**(스노우싸인 `template_draft` 임베드 iframe — PDF 업로드·서명칸 배치·역할 정의는 스노우싸인 안에서, 앱은 좌표 미저장. 완료는 postMessage 자동 + "등록을 마쳤어요" 수동 폴백[template_id 입력]) → **mapping**(getTemplateDetail 로 불러온 템플릿 역할명 ↔ 구매사/PG 서명자, 변수명 ↔ 낙찰 견적 소스[정산주기·정산한도·가입비·보증보험 등] 연결 → linkSigningTemplate 저장. **저장은 어떤 계약도 발송하지 않는다** — 기본 템플릿(`is_default`) 개념은 제거됐고 발송은 딜룸의 명시적 확인뿐이다). **org 스코핑**: 단일 API 키=1 org 라 `GET /v1/templates` 원본 비노출, `pg_signing_templates` 링크분만 표시(타 PG 템플릿 차단). nav top 레벨 (G→E). 세 액션(임베드 세션 발급·detail 조회·저장)은 `run()` 헬퍼가 try/catch + finally 로 감싼다 — throw 를 삼키지 않으면 `busy` 가 true 로 남아 화면의 모든 버튼이 새로고침 전까지 영구 비활성이 된다. | `SigningTemplateManager`, `Select`, `PageHeader`, `EmptyState`, `Note`, `DropdownMenu`, `Dialog`, `ConfirmDialog` |
