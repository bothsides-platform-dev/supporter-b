...
별도 `admin-supporter-b` 콘솔의 `/pg-recommendations`는 업종 이름·표시 순서·PG 연결을 편집한다. MCC 기반 상담용 업종 29개를 이름·코드로 검색하고 복수 선택해 일괄 등록한다. 기존 코드·동일 이름은 건너뛰며 기존 업종 ID와 정책을 보존한다. 기본 추천 PG 영역에서는 활성 PG와 우선순위·사유·확인된 요율을 저장한다. 업종별 후보가 없을 때만 기본 후보를 사용하고 접수 불가 업종과 이전 상담 PG는 제외한다. PG 한 곳을 여러 업종에 연결할 수 있으며, 변경은 구매사 작성 화면의 추천에 반영된다.

**구매사 업종 입력 (2026-09-24)**: 실제 견적 작성은 업종 질문을 항상 표시한다. 구매사가 업종 이름의 라디오 목록에서 하나를 선택하면 해당 `industryGroupId`를 초안에 저장하며, 선택은 이전·다음 이동과 새로고침에서 유지한다. 업종 목록이 비면 질문을 생략하지 않고 목록 재조회·운영팀 문의를 제공하며 다음 진행을 막는다. 재조회는 작성한 답변을 보존한다. 업종을 건너뛴 기존 초안은 마지막 질문에서 업종 선택으로 돌아오며, 유효한 선택 없이 추천 단계로 넘어가지 않는다. 업종 선택지는 관리자 등록 목록만 사용하고 임의 ID나 자동 분류로 대체하지 않는다.
