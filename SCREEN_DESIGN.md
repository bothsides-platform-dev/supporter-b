...
- 헤드라인: `구매사 계정을 만듭니다`
- 이메일 입력, 실시간 형식 검증
- 회사 이메일 권장 안내(`SignupEmailGuide`): 인풋 아래 상시 중립 힌트 "회사 이메일로 가입하면 팀원과 함께 쓰기 쉬워요." → 무료(개인) 도메인(gmail/naver 등, `lib/auth/free-email-domains.ts`) 감지 시 amber 경고 한 줄로 전환 "개인 이메일이에요. …" (비차단, EMAIL_TAKEN/마스터 에러 표시 중에는 숨김)
