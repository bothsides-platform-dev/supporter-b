...
- **RFP 작성 2단계 발송 필수 필드 — `견적 유형`·`주요 판매 상품`·`전년도 연간 PG 총 거래액`**: 작성 위저드 2단계의 세 필드는 **발송 시 필수**다(제목·홈페이지와 함께). 작성 도중(draft)에는 비워둬도 되지만 발송하려면 채워야 한다 — 견적 유형은 신규/갱신 중 하나, 주요 판매 상품은 비어 있지 않은 문자열, 연간 거래액은 0보다 큰 정수여야 한다. 판정 로직은 클라이언트 위저드와 서버 `createRfpAction` superRefine 이 **단일 출처(`lib/rfp/required-fields.ts`)** 를 공유해 드리프트를 막고, draft 는 허용하되 발송(send)에서만 강제한다(서버가 trust boundary). 필드에는 `RequiredMark` 칩(empty/filled/error 3상태)이 붙는다.
...
| B2 | `/rfp` | RFP 목록. 진행중/마감 탭 (v0.2.54.0에서 '선정 완료' 탭이 '마감'으로 통합됨 — 결과는 마감 컬럼 안의 칩(선정완료·미선정·취소)으로 구분. 작성중 단계는 제거 — draft RFP는 `?status=draft` URL/표로만 접근). **온보딩 샘플**: 신규·기존 구매사는 `isSample=true` 샘플 견적 요청 1건을 목록 최상단에서 볼 수 있다(`샘플` Chip 표시). 목록 행에서 직접 삭제 가능(삭제 영속 — 재시드 안 함). | `RfpList`, `DataTable`, `Tag`, `SampleRfpBanner` |
